...
The shared tsdown preset emits `lib/client.cjs.map` for every plugin and rewrites first-party source paths into the browser-resolvable repository shape `/packages/<group>/<package>/src/...`. Other workspace sources inlined into a bundle likewise resolve to their `packages/` owner, while dependency paths remain unchanged; `sourcesContent` carries the source. The Host rewrites the served bundle's terminal reference from physical `client.cjs.map` to public `client.js.map`, serves that URL from the physical map, and exposes no source route. The Vite shell also emits source maps, letting both shell code and out-of-graph plugins map stacks and performance profiles back to TypeScript/TSX.
