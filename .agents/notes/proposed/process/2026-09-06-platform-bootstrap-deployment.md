...
Host cleanup relies on a container ownership label written during bootstrap activation; losing that label makes cleanup fail closed and leaves durable recovery state for an operator. The EIP pair is a narrow dispatch input rather than a global inventory and remains valid only for the reviewed bootstrap transaction. Infrastructure readiness, NAT, Redis, certificates, and EIP routing remain external prerequisites; this workflow does not provision them.
