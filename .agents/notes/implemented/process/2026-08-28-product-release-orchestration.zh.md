...
- [`product-workflows.spec.ts`](../../../../scripts/release/product-workflows.spec.ts) 固定 pull-request CI 不运行 release-intent 校验、不产生发布副作用的 Product Release PR 生成、可复用工作流的最小权限、可信执行版本、精确候选与版本输入、命名产物恢复、直接渠道失败传播、持久 Mobile prerelease、Platform 部署与仅发布恢复、选中 lane 调用与最终 manifest 产物。
