...
Product Release Plan still consumes optional `.release-intents/*.json` records when they exist on `master`. Ordinary product pull requests do not add or validate those records. CI does not fail a ticket pull request for a missing intent.

When unconsumed optional intents exist on `master`, `Product Release Plan` creates or updates `automation/product-release`. The Draft Product Release PR consumes each remaining intent once, applies the highest requested bump to every selected unit, increments the source-owned Mobile build number, filters bilingual summaries by selected release unit, and commits release notes plus a numbered plan under `product-releases/`. CI recomputes this transaction from the base ledger, versions, Mobile build, and unconsumed intents, then rejects any forged plan, state, version, tag, bump, summary, build, or Desktop note. Application versions remain in their owning package manifests. `apps/mobile/release.json` owns the monotonic Mobile build number; native packaging reads the Marketing Version from `apps/mobile/package.json` and the build number from that tracked file rather than GitHub Environment variables.

Desktop release notes require the preceding selected version's `gestalt-v*` tag. The generator resolves that tag before changing any source authority and reports that the preceding Desktop release must be published or recovered when the tag is absent. The planning workflow propagates generator failure through output capture, so a failed preparation cannot add, commit, push, or publish a partial planning transaction.

One `Product Release` coordinator checks out the explicit full candidate commit, verifies that it remains the latest valid plan in the reachable master ledger, validates every source-owned version, and then calls independent reusable Desktop, Mobile, Platform Image, and Platform Deploy workflows. The coordinator and every workflow that can promote or push independently require both the currently executing `github.sha` and the requested candidate to be full commits reachable from current `origin/master`; only a Platform Image verification build with `push=false` is nonpromoting and exempt. Each workflow builds once at the plan commit, records artifact or full OCI digests, and promotes those exact bytes after its owning GitHub Environment approves the release action. Recovery is an explicit input. Desktop candidate assembly downloads its three named pack artifacts in sequence and never enumerates peer-lane artifacts from the coordinator run. Desktop manual recovery has no mutable version default: its tracked package manifest owns the version, and the plan binds that version to the candidate. Every prior run must be completed in the same repository through an allowed workflow, while the exact named producer job must have succeeded and its candidate-named artifact must remain unexpired. Its workflow head must independently be a full commit reachable from current `origin/master`; equality with the requested candidate grants no exception. An unmerged branch cannot supply trusted workflow code, while a later trusted coordinator or lane failure does not invalidate already-produced bytes. Downloaded manifests and recomputed digests separately bind Desktop and Mobile artifacts to the requested candidate; Mobile can upload the validated prior IPA and preserve validated prior TestFlight evidence. The Mobile publication job evaluates its direct successful prerequisites under `always()` so the intentionally skipped member of the mutually exclusive build and recovery paths cannot suppress GitHub publication. Platform recovers image and deployment identity only from the corresponding successful producer and candidate-bound metadata; requested identity is never evidence. Requested channel publication failures fail the direct lane workflow, while the coordinator uses its final `always()` job to record released, skipped, and blocked units, require a reason for every blocked unit, separate Actions run URLs from Release URLs, and record a TestFlight build only when an evidenced upload ran.
...
- [`product.spec.ts`](../../../../scripts/release/product.spec.ts) pins bilingual release-intent parsing, unknown-field rejection, application dependency closure, explicit input mapping, under-report rejection, scoped aggregate exceptions, unique unconsumed additions, merge-group aggregation, base-ledger plan and state recomputation, unpublished Desktop baseline rejection before source writes, generated-field isolation, latest-master candidate validation, named producer and artifact provenance, signed-candidate digests, per-surface summaries, Mobile build increments, candidate-bound full Platform digests, and final manifest rendering.
- [`product-workflows.spec.ts`](../../../../scripts/release/product-workflows.spec.ts) and the Desktop workflow tests pin that pull-request CI does not run release-intent validation, fail-fast Product Release planning with captured output, Product Release PR generation without publication, least-required reusable-workflow permissions, trusted executing revisions, exact candidate and version inputs, sequential named Desktop artifact downloads, named-artifact recovery, direct channel failure propagation, Mobile publication after an intentionally skipped producer path, durable Mobile prereleases, Platform deploy and publish-only recovery, selected-lane invocation, and final manifest artifacts.
...
Ordinary ticket pull requests no longer carry SemVer impact in-tree. Maintainers still choose Desktop, Mobile, and Platform bumps when assembling a Product Release Plan from any optional intent records that remain on `master`.
