...
两个标准 hosted Windows worker 各自拥有 8 个 single-worker 覆盖率分区中的 4 个，每次运行 2 个分区进程，并且只发布 Vitest blob。两个 worker 合计 4 个并发分区进程，在保留并行覆盖的同时，不会让单台 runner 内的 SQLite 与子进程测试因资源匮乏而超时。最后一个标准 Windows job 下载完整 blob 集合，只应用一次全仓阈值，并运行豁免重型清单。现有故障切换池采用相同的双 shard 拓扑。构建/runtime 与静态可移植性继续作为独立标准 hosted job。每个 job 都有独立的 immutable install 和 20 分钟 timeout，因此 setup 重复换来了互不共享可变构建产物的独立时钟。
