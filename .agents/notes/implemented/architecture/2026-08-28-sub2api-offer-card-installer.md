...
The controlled restart ([`replaceWebHost`](../../../../apps/desktop/src/main.ts), also used by the crash-respawn path) stops the child, spawns a fresh one through the existing [`spawn-web-host`](../../../../apps/desktop/src/spawn-web-host.ts) seam, reloads the window and the native overlay onto the new port-0 URL, and re-points Companion RPC; sessions survive on disk. Only the first restart after installation supplies a 180-second startup deadline matching the supervised bootstrap budget; re-enable, disable, uninstall, ordinary startup, and crash recovery retain the 30-second deadline. The desktop bundle grows by `tar`, `js-yaml`, and `@deepseek-ai/dsh-home-paths` — the install stays inside the app, and the installer size never carries the runtime pack.
