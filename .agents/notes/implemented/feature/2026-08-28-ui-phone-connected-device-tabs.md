...
**Play the signed H264 URL with a `<video>` element.** Rejected: the Host proxies a raw `avc` elementary stream, which no browser `<video>` decodes without MSE/WebCodecs remuxing. The live view still loads that signed URL as the capture element; decoder work stays with the Host/engine ticket.
