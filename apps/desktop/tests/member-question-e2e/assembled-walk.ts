...
export async function runAssembledProjectMembersWalk(): Promise<string> {
  ...
  try {
    ...
    return [
      ...
      { type: 'later-terminals', outcomes: ['expired', 'withdrawn', 'superseded', 'MEMBER_OFFLINE'] },
      { type: 'wire-floor', retained: 'ciphertext-only' },
  ...
  throw new Error('assembled Project Members walk returned no transcript')
}
