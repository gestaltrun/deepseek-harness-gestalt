...
纯 React 原子组件（零 cordis）：StateDot、DisclosureRow、ic_ds_* 图标、Button/Pill/Menu/Modal/Input、Menu 与 Modal 持有的 overlay 锁（Desktop 据此把 Host chrome 抬到官方页面视图之上）、Toast 短时横幅、OnboardingSurface 首次使用接管层（portal 到 body 的遮罩加不透明展示层，在且仅在自身生命周期内保持 `#root` 为 `inert`）、markdown 家族（MessageText/MarkdownText/JsonBlock）、只读 JsonTree 检查器、`useAnchoredMaxHeight` 钩子（把底部锚定的浮层高度收敛到锚点上方的视口空间，并在 resize、scroll 与调用方提供的依赖变化时重新测量）、`useAnchoredPosition` 钩子（让固定定位的浮动面板跟住锚点任一边：测量、偏移、按视口边距钳制，并在捕获阶段滚动、窗口缩放与面板自身尺寸变化时重新定位；可选宽度用于在 portal 挂载前完成定位）、`useDismissOnOutsidePointer` 钩子（把触发器根节点与可选 portal 视为同一个自有表面）、TerminalBlock、DiffBlock、ReadBlock、SearchBlock，以及 WebBlock。
