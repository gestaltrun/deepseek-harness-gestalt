...
`MobileAccount` owns one signed-in screen state: home, account, or pairing. Home contains a 40-pixel Account menu entry, selected Desktop status, and shared Workspace and Session rows without exposing account identity in the Session header. Account and Personal Pairing occupy separate full-height pages with explicit back navigation. The initial pairing task gives QR scanning the primary action and reveals complete-link paste only as an explicit fallback; handshake progress, security words, retry, and unavailable states replace that task instead of stacking beneath it. Its ready view keeps the hero, camera preview, and complete-link form in one bounded method stage; choosing another method replaces that stage, while its actions follow the task copy instead of anchoring to the viewport floor. Search occupies a focused page that renders only Desktop-authoritative results and returns to the Session list. Workspace and Ungrouped creation occupy a labelled pending page until Desktop publishes the created Session id; Remote Offline or failure cannot expose an editable conversation. Search and creation destinations remain navigable while Remote Offline, but their Desktop operations stay fail-closed until foreground synchronization returns. The home viewport owns one overlaid bottom action dock, each Workspace header independently folds its rows, and each group owns its own bounded row window and load-more control. Desktop archive authority removes Sessions from the initial list, live replacements, and full-text results. The native applications admit portrait presentation only. The complete signed-out surface, including retention facts, consent, progress, action, and footer, follows the selected Chinese or English locale while retaining both privacy notices. The shell submits activation and deactivation at each Account transition; the pairing controller serializes those operations by generation, so a slow old activation and cleanup cannot stop the next signed-in generation. `MobilePairing` disables its own lifecycle ownership when mounted inside that shell.

The Account page places GitHub identity beside the avatar and owns a Chinese/English selector persisted for the mounted Mobile product. The Session list admits pull-to-refresh only from its scroll origin. Remote Online sends one encrypted `refresh-surface` operation at offset zero and replaces the list from the correlated Desktop projection; Remote Offline sends no operation and explains that reconnection is required, including after the Companion Cache has been cleared.

Conversation detail remains a full-screen destination over the shared `ui-conversation` components and the shared `ui-workspace` Session rows. A blank detail labels itself New Session instead of exposing its opaque Session id; the phone header owns an icon back action, and the encrypted attachment action occupies the shared InputBar's leading tool row. The design prototype supplies layout reference only. Bundled-entry snapshots and checked-in Capacitor applications remain the product acceptance entries; `prototype-companion` and ports 5173/5174 provide no acceptance evidence.
