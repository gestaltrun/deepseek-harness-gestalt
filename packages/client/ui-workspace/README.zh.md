...
Workspace 行的 ⋯ 菜单以**工作区设置**为首项。设置弹窗承载工作区升级区块：先是通过名称与 remote 校验（remote 必须是 http(s) 代码仓库地址）创建云项目，然后是对已绑定项目的成员管理——每行名册的显示名、角色选择、职能标签编辑、在线点和移除——以及按 GitHub 登录名邀请和可撤回的待确认邀请行。所有操作都经由可选的 `projectMembership` gateway prop 路由，由组合层适配 `@deepseek-ai/dsh-project-membership-client` 并走 `/v1/projects` 路由；没有该 gateway 的组合仍显示菜单项，但不渲染升级主体。15 秒一次的待确认邀请轮询驱动两步邀请向导：邀请卡（接受/拒绝），随后是必选的本地工作区关联步骤——remote 与邀请一致的工作区标注「同源推荐」，已知不同源标注「异源」，**新建克隆…** 始终可选；没有「暂不关联」，关闭向导不做任何决定，邀请回到待确认池中。
