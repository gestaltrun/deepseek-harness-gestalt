...
const ROSTER_TEXT = JSON.stringify([
  ...
  { accountId: 'acc-demo-dev', displayName: 'Demo Member', role: 'member', tags: ['review'], presence: 'offline' },
])
...
describe('project_members keyless assembled transcript', () => {
  it('boots the composition and reads the roster through the real agent loop', async () => {
    ...
    expect(calls).toEqual(['project_members'])
    const resultEvent = events.find(event => event.type === 'tool/result')
    const message = (resultEvent?.data as JsonObject | undefined)?.message as JsonObject | undefined
    ...
    expect(assistantText(events.filter(event => event.type === 'assistant/message').at(-1)))
      .toBe(`PROJECT_MEMBERS_ROSTER ${ROSTER_TEXT}`)
    expect((records.at(-1) as JsonObject).output).toBe(`PROJECT_MEMBERS_ROSTER ${ROSTER_TEXT}`)
