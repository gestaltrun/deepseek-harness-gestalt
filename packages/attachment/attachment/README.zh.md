...
持久附件服务边界。`ctx.attachments` 校验并持久提交不可变图片或通用文件字节，随后返回可序列化的内容寻址引用；消费方绝不会在会话事件中持久保存浏览器路径、对象 URL、提供方 URL 或 base64。`saveFile` 与 `readFile` 保留确切字节、有界 media type 与展示名称、字节数和 SHA-256。不支持通用文件的提供方会明确拒绝该操作。
