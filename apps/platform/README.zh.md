...
部署会为每台 ECS 提供不同的 `PLATFORM_RELAY_INSTANCE_ID`，并通过 `PLATFORM_RELAY_*` 变量提供正数的容量、确认等待、directory TTL、heartbeat timeout、ciphertext buffer、连接数、待投递数与 attach timeout。PostgreSQL 拥有一次性附件 capability digest、pairing id、私有 OSS object key、byte length、expiry 与容量；私有 OSS bucket 保存密文字节，但不会收到端点密钥或明文。上传会在发布元数据 authority 前完成。consume、revoke 与 expiry 会先提交元数据删除，再清理对象；限定在 `PLATFORM_OSS_OBJECT_PREFIX` 的一天 lifecycle rule 会回收直接清理失败的对象。部署 preflight 通过 ECS IMDSv2 读取临时凭证，保留 bucket 上无关的 lifecycle rule，并在替换运行中容器前校验或安装精确的 namespaced rule。每次读取持久行时都会校验 digest 长度、品牌化 pairing id、精确 object-key binding、有界 byte length 与安全整数 expiry，校验通过后该行才能进入产品 authority。附件 HTTP 会鉴别当前 Mobile Installation 与确切的已确认 pairing selector；selector 本身没有 authority。应用会在监听前校验完整配置。
