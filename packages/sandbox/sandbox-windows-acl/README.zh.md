...
模式（令牌的 restricting-SID 列表随模式而变；保活组在调用者令牌具备登录 SID 时包含它，并在**两种**模式下都包含 Everyone；非交互式服务令牌可能没有登录 SID，因此仅保留 Everyone。可用的保活 SID 若被省略，早期 DLL 初始化会以 `0xC0000142` 死亡、CNG 会让 pwsh 以 `0xE0434352` 崩溃）：
- `workspace-write`（可选登录 SID、Everyone、工作区 SID、临时 SID）：工作区与会话的**私有**临时子目录分别携带 Write 授权；受 ACL 管辖的其他写入都会被拒绝，已记录的 Everyone 与硬链接边界除外。
