...
Side Chat 标签页在 Host 重启后存活。线程是持久化子 Session，而标签条存放在按 origin 隔离的 localStorage 中，因此以新 origin 重启（桌面端的临时端口）只会让标签条失孤，线程本身仍在。当某个会话的侧栏状态激活时，标签条会与会话列表对账：该会话所有已发布且没有在线标签页的侧边线程都会恢复标签页，落在活动 pane 中且不抢占其活动标签，收起的面板会重新展开以便恢复可见（窄屏的全屏抽屉保持关闭）；从未提问的空白线程和仅渲染器侧的临时身份不会恢复。关闭 Side Chat 标签页会在持久化状态中为该线程记下墓碑，因此被用户关闭的会话在重启后保持关闭；`?dsh-sidebar-reset` 逃逸参数会在本次加载中跳过恢复。
