...
账号隔离的受保护 pairing 文档使用版本 2。它原子保存一组有界独立 pairing 记录，以及一个可选 selected pairing id。每条完整记录拥有自己的 Personal Pairing id、Mobile Relay grant、Snow reconnect state、attachment key，以及只从已鉴权 `foreground-sync` 获得的可选 Desktop 名称。该文档会拒绝其他版本、缺少完整权限的 selected id、畸形 secret 字节和超过 retained-pairing 上限的记录。Account 选择会串行执行 load 与 save，把每份 secret 复制进 active scope，并清零转移来的 buffer。

完成 Personal Pairing 会选择这台新 Desktop，因为 pairing action 本身就是显式行为。之后的选择是另一项由人触发的 action。`MobilePairingController` 在持久化或 attach 另一个 grant 前，会让当前 mutation authority 失效，释放上一组 projection 与 cache binding 但不删除已存内容，并排空旧 Relay lifecycle。随后，它持久化 selected pairing，只配置该 pairing 的 grant，完成一条新的 pairing-specific Snow IK channel，并且只接受该 channel 的 Session projection。失败会让 mutation authority 保持 disabled 且错误可见；产品不能在新 selection 标签下继续使用旧 channel。
...
Vault 测试会通过 IndexedDB 与原生受保护存储 adapter 持久化两组独立 grant、名称、密钥、reconnect record 和显式 selection，并证明只释放所选记录及账号隔离。Controller 测试证明在激活新 grant 前会释放旧 projection 与 Relay，只在 Platform 撤销所选记录，保留另一组 pairing，并且在移除后不做隐式选择。已鉴权 receiver 测试证明，只有加密 Desktop projection 能提供保留的名称。打包 Mobile snapshot 会先渲染 selected 与 unselected Desktop 控件，再执行共享 Session 与 conversation 组件。
