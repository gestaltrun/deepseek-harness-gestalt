...
Issue Priority synchronization is an explicit deployment option in `.github/issue-management/config.json`. Set `priorityField` to an organization Issue field name only when the repository supports that field; the policy then reads every referenced Issue's value and fails on API errors. Set it to `null` to disable the integration: the policy makes no Issue field request and treats referenced Issues as having no Priority. The Gestalt tracker uses `null` because its workflow has no configured Issue-field authorization for Priority synchronization.

Organization Project lifecycle projection is an explicit deployment option. Set the repository variable `DSH_ISSUE_PROJECT_LIFECYCLE_ENABLED` to `true` only when the repository owner matches `projectOrganization` in `.github/issue-management/config.json`, that configuration names the target organization Project, and the repository provides `DSH_ISSUE_APP_CLIENT_ID` and `DSH_ISSUE_APP_PRIVATE_KEY` for an installed GitHub App with the required repository and organization permissions. The workflow validates the shared owner before requesting an installation token. When the variable is absent or not `true`, owner validation, token creation, and Project mutation steps skip while the job remains successful. Lifecycle and audit requests always require the App token; `pullRequestReadAuthentication` never makes their reads or writes anonymous. The Gestalt organization tracker leaves this option disabled because no Project identity or GitHub App authorization has been confirmed. The current `projectOrganization` is a disabled sentinel that intentionally differs from the repository owner, so accidental activation fails during owner validation before token creation; a future deployment must replace it only after confirming the target Project and credentials.
