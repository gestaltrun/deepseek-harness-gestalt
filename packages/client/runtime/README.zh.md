...
成员提问接收 Session 在完整生命周期内都仅用于渲染，但其 identity 与 record 归 Host 所有。`ReceivingQuestionBook` 只应用 revision 更高的完整 `host/member-question-snapshot` 帧，使用每条持久化 `ReceivingSessionId`，并通过对外 `SessionFace` 发布 pending wait 与 terminal record band。断连会保留最近的 Host 状态；重开的 stream 会重新建立 pending 与 terminal 行的基线，且不改变其 id。Client 绝不裁决 expiry、supersession、withdrawal 或 settlement，回答与拒绝动作会调用 `memberQuestion.settle`。该 face 没有 Chat、队列、历史、模型、命令、skill 或变更路由，其 composer phase 为 disabled。选中、重连与 renderer 打开都不会为该身份创建或刷新 Host Session。
