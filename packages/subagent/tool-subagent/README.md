...
| `deploymentRoutePreauthorization` | `false` | Union the current deployment registry snapshot into each fresh top-level Session; independent of Settings and requires provider `agentOptions` support |
...
Set `modelSelectionSettings: true` to sample the Host's enabled `subagent-model-selection` preference when each fresh top-level Session is composed. Set `deploymentRoutePreauthorization: true` to independently sample `ctx.subagentRoutePreauthorization`; this mode requires no Settings service and waits to register the route-selectable tool until that Provider is mounted. When both are enabled, the tool records their sorted, deduplicated union. A restored Session without a recorded policy remains disabled, including an explicitly empty restore. Recorded policy is inherited by child Sessions and remains unchanged by later Settings edits or Provider replacement; a Provider detach temporarily removes its tool registration, and reattach restores the same durable policy without another event. The tool then exposes optional `provider`, `model`, and `reasoning_effort` fields and registers the shared `list_subagent_models` tool. Either mode requires a backend that advertises `agentOptions`; both in-process backends and DSH SDK support it, while ACP, Codex, and Claude Code reject it rather than ignore it.
