...
describe.skipIf(MODE === 'record')('web e2e: Host-owned member-question receiving session', () => {
  ...
  it('answers and declines through Host authority while retaining terminal bands', async () => {
    ...
    try {
      ...
      await expect.poll(() => page.getByText('.dsh/member-questions/mq-web-host-1/receiver-decision.md').count())
        .toBeGreaterThan(0)
      expect(await page.locator('[data-details-panel]').count()).toBe(0)
