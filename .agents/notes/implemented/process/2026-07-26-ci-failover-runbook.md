...
`ci-master.yml` sets `PNPM_CONFIG_OPTIONAL=true` for every job and runs the standby installs with `--force`. The complete standby aggregates execute product tests and static tools whose platform executables are optional dependencies. The persistent runners can retain a `node_modules` tree created while optional dependencies were disabled, so each drill must rebuild that tree under the repository-owned setting.
