...
- **MIME 表跟随实际交付的资产类别**：Web 构建、PWA manifest 与爬虫发现文件之外的扩展名会回退到 `application/octet-stream`，直到应用实际需要它们。
