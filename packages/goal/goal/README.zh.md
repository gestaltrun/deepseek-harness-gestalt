...
续行启用状态绝不持久化。新缓存与每次触发 `agent/session-start` 时都会停用续行，即使回放找到了持久 phase 为 active 的目标。续行驱动器在卸载前或持久性不确定后也会调用 `disarm()`。因此，会话恢复、携带种子历史的自动化子代理与驱动器替换会保留目标、phase、revision 和已准入 Round 数量，却不会启动工作；之后必须通过显式 resume 变更重新启用续行。

`clearInherited(agent)` 是 fork 边界专属动词。以父日志前缀为子会话种子的产品级 fork 会在创建时调用它一次：当子会话的当前目标完全来自种子前缀、且子会话自身未追加任何目标变更时，它会提交一条 clear 墓碑，使 fork 出的线程以无目标状态开始。已经自行变更过目标的子会话（包括先前清扫写入的墓碑）保持原状。持久墓碑保证清扫幂等：回放与后续恢复读取墓碑即可，绝不重复执行清扫。
