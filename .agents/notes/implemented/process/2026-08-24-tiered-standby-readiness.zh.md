...
完整的非分片 Linux 与 Windows 清单按日运行，也可以通过显式的 `standby-exhaustive` workflow dispatch 启动。它们会在运行串行聚合流程前执行相同的干净安装，timeout 为 120 分钟。持久 Windows service host 的执行策略禁止加载仅由重复度分析使用的 optional native resolver，所以该聚合流程省略这些由 Linux 权威覆盖的静态 gate，同时保留全部 Windows 特有 build、coverage、产品、文档与仓库 gate；hosted Windows 仍运行完整静态清单。其报告会把失败分类覆盖为 `failover-readiness`；有界 smoke gate 则直接携带该 failure domain。因此 CI 指标会保留这些证据，但不会把 standby 失败计为产品回归。
