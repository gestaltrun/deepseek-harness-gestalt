...
`packages/*/tool-*` 下已发布的产品工具包，每个都使用默认配置启动，包括 `dsh-tool-bash`（`bash`）、`dsh-tool-jobs`（`job_output`、`job_list`、`job_kill`）、`dsh-tool-phone`（`device_act`、`device_close`、`device_list`、`device_observe`、`device_open`、`device_screenshot`）和 `dsh-tool-subagent`（`subagent`）。仅供示例使用的工具不在范围内。
