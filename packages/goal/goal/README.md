...
`clearGoalFromForkSeed(seed)` prepares a product fork before publication. When the contiguous parent prefix ends with a current goal, it returns a new seed with one trailing clear tombstone; otherwise it returns the original seed. The host records the parent-prefix length as `header.seedLength`, so the tombstone is a child-owned event even though creation admits the complete sequence atomically. Automation providers that do not call this helper retain seeded goal state.
