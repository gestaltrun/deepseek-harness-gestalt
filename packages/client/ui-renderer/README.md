...
The client entry also owns the React implementation of slot outlets, session providers, and observable-to-uSES binding. Business plugins pass bare observable sources through typed slot `hooks`; the renderer binds them at the outlet. The plugin activates after `slots`, `sessions`, and `layout`, projects the selected session title, and mounts the application through `renderSlot('root')`. Its `mountSession()` service operation lets a feature shell mount one declared non-root Session slot under an explicit Session id without changing shell selection; the caller owns the returned independent-root disposer. React, React DOM, Cordis, ui-slots, and ui-primitives retain one browser identity through the web shell's static module table; this package arrives as a dynamic client bundle.
