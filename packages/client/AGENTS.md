...
5. Rebuild the bundle (`pnpm --filter <pkg> bundle`) before probing a live `dsh web` server — the registry serves `lib/client.cjs` through `/plugins/<id>/client.js`, not sources.
