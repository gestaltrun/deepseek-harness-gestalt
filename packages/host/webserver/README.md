...
Web HTTP and upgrade-route registration plugin (default-exported `WebServer`, config `{host, port}`): a `node:http` server that listens on activation and provides `ctx.webServer`. Named exports `readJsonObject`, `writeJson`, `writeHttpError`, `writeRetryAfterError`, and `HttpError` give route owners a bounded JSON object reader and no-store JSON error writers; each caller supplies its own status, code, and message. `CorsOriginPolicy` validates a caller-selected exact origin list, preserves custom tuple origins without admitting opaque `null`, and returns only configured matches; route owners still choose the policy and protocol error. `register(route)` adds a named `exact`/`prefix` HTTP route; `registerUpgrade(route)` adds an upgrade route for an exact pathname. A duplicate path within either table throws because route patterns are a composition-level contract and a collision is a misconfiguration; both methods return a disposer that removes the registration. `registerFallback(handler)` registers the one handler for requests that match no named route. A second registration throws; the SPA dist server [`dsh-host-frontend-static`](../frontend-static/README.md) is the shipped owner, and the server returns 404 while none is registered. Index startup input is a structured row table: `collectIndexInjections()` collects a fresh `IndexInjection` table through one `webserver/index-inject` emit per call; `renderIndex(html)` renders the rows into an index response and then applies raw `tapIndex(transform)` transforms in registration order through `applyIndexTaps(html)`. The fallback owner calls `renderIndex` for each index response, while static deployments carry the same rows in the boot payload and render them with the exported `renderIndexInjections`. `port` reads the listening port (the OS-assigned value when `port` is 0), and `host` reads the configured bind host (composition-time facts other plugins adapt to, e.g. the directory-picker chooser). HTTP match order is fixed: exact over the whole table, then longest prefix, then the fallback handler. Upgrades match exactly and unmatched connections are closed; registration order carries no request-facing semantics.
