...
The browser-safe `dsh-session/surface` subpath owns `contextProvenance()`, `contextForm()`, and `sessionRecallLabels()`. Web imports the same pure projections that Desktop uses when producing bounded Companion values, so Mobile's shared Web components and Desktop cannot drift into different role, label, or form rules.
...
- Context-source presentation policy is public session projection API because both the Desktop Companion producer and Web presentation consume it; changes update the session package and subsystem documentation together.
