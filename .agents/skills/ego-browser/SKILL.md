...
Record the live DSH task space for this goal in `.agents/local/runtime-memo.json` under `ego` (`profile`, `taskSpaceId`, `taskSpaceName`). That file is gitignored local inventory shared with [dsh-desktop-test-instance](../dsh-desktop-test-instance/SKILL.md). Read it before the first heredoc of a round. If `ego.taskSpaceId` is present, call `listTaskSpaces()`, require that id under the `DSH` profile, and pass it to `useDshTaskSpace`. Create a space only when the memo has no `ego` record and `listTaskSpaces()` has no DSH space for this goal. After create or reuse, write the numeric id back to the memo. Complete a round only when this goal still has exactly one DSH task space; creating a second space while the first still exists is a failed round.

For any follow-up on the same user goal — including continue, corrections, retries, validation, user-reported problems, or work after `completeTaskSpace(..., { keep: true })` — resume the original task space first if it still exists. Do not create a new task space for the same goal unless the user asks for a fresh space, starts an unrelated goal, or the original space is unavailable after checking. If a new space is necessary, state why and replace the memoed id.
