...
`projectByRemote` 把 HTTP 204 与生产环境的 HTTP 404 视为未绑定。其他非 OK 应答仍会拒绝。Workspace settings 独立加载当前 Account 的 Project 与本地 Git remote，因此未绑定的 404 不会清掉已成功读到的 origin。`createBlocked` 只看名称：Git remote 为可选项，有值时只读。
...
- `packages/platform/project-membership-client/tests/membership-client.client.spec.ts` 钉住生产 404 为未绑定。
