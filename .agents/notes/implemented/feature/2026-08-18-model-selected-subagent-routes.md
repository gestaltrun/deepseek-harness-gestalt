...
`dsh-tool-subagent` exposes optional `provider`, `model`, and `reasoning_effort` fields only when its Agent-scoped instance records a non-empty Session policy and the bound subagent provider advertises `SubagentCapabilities.agentOptions`. A fresh top-level Session records the deterministic union of an enabled user authorization from [user-authorized subagent model routes](2026-08-24-user-authorized-subagent-model-routes.md) and the current snapshot from `ctx.subagentRoutePreauthorization`; either source may be enabled independently. The deployment service and its static Provider are separate packages, the Settings owner has no deployment defaults or writes, and Provider contributions are effect-scoped. Disabled instances omit and reject model-facing selection. A route-selectable instance against a provider without the capability fails the plugin mount.
...
- The primary delegation-tool instance defaults selection off, exposes a Plugins-page exact-route opt-in for new Sessions, and registers `list_subagent_models` only in Sessions whose durable policy exists; discovery and effective route selection are constrained to that policy.
