...
Side Chat tabs survive a Host restart. A thread is a durable child Session while the tab strip lives in origin-scoped localStorage, so a restart under a new origin can lose the strip without losing the threads. When a Session's sidebar state activates, the strip restores every published, unarchived direct Side Chat child that lacks a tab. Restored tabs land in the active pane without replacing its active tab; blank children and renderer-only provisional identities do not restore. Closing a published Side Chat archives its durable Session without deleting its log, while a local tombstone prevents list refreshes from reopening the tab before the archive projection arrives. The `?dsh-sidebar-reset` escape hatch skips restoration for that load.
