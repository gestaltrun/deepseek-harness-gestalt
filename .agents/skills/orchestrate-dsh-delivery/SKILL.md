...
4. Ask each writer session to run [`retro`](../retro/SKILL.md) on its own session. Collect the candidates in the root task, present a synthesized list to the user, and wait for an explicit keep-or-drop decision per item. Dispatch a writer to land only accepted environment changes on the specification branch, then accept the writer's re-run of the affected checks. Do not merge to `master` before that decision.
