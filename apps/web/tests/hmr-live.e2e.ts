...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  ...
  const originalClientArtifacts = await Promise.all(clientArtifactPaths(REPO_ROOT)
    .map(async path => [path, await readFile(join(REPO_ROOT, path))] as const))
  ...
  try {
    ...
    await Promise.all(clientArtifactPaths(REPO_ROOT).map(async (path) => {
