...
`phone-stream` 在与会话铸造相同的 `/api` 信任栅栏之后（仅 GET、精确路径）应答 `GET /phone/devices`：handler 调用 `ctx.phoneDevices.listDevices()`，把每个设备条目投影为文档化的 `id` / `name` / `kind` / `online` 响应字段，按 `android` / `ios.simulators` / `ios.reals` 分组。投影是显式的：runtime 分组条目物理上携带上游 `platform` 字段，而公开类型 `PhoneDeviceRef` 早已擦除它；原样转发等于把该内部字段烤进新的响应体。
