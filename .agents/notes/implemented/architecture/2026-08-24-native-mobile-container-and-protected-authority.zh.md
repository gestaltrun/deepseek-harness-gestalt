...
[Mobile Companion 提案](../../proposed/feature/2026-08-17-mobile-companion.zh.md)选择了轻量原生容器、受保护配对密钥、加密离线内容、相机配对与受控分发，但产品目录只有 Web 源码。实际运行入口通过浏览器存储持久化 Installation 身份与配对权限，Companion Cache 尚未接入，也没有可重复的 App Store 或 Android release-key 产物链路。因此，Web build 成功无法证明原生存储、升级、lifecycle、picker、签名或打包入口行为。

## Decision

`apps/mobile/ios` 与 `apps/mobile/android` 是 `com.gestalt.mobile` 的仓库内 Capacitor 工程。Capacitor 会把编译后的 `apps/mobile/src/main.tsx` closure 复制到每个应用；两个工程都不会从 Desktop、Platform、Vite 或 `prototype-companion` 加载可执行应用代码。两个原生工程都注册 `GestaltProtectedStorage`；由于原生 bridge callback 可能包含解密后的受保护值，所有构建都关闭 Capacitor 日志。iOS 使用 `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly` 把 UTF-8 值保存为 generic-password Keychain 条目。Android 在 Android Keystore 中创建不可导出的 AES-GCM 密钥，把每个加密 SharedPreferences 值及其存储 key 绑定为 additional authenticated data，每次替换都使用新的系统 IV，并禁用应用备份。产品 composition 要求该原生插件，并在其中保存稳定 Installation id 与一份版本 2 的账号隔离 pairing 文档。该有界文档会保留每个 Personal Pairing 的 Mobile Relay grant、96 字节 IK reconnect record、attachment key、已鉴权 Desktop 名称、待处理配对恢复，以及[显式选择的 Paired Desktop](../feature/2026-08-24-explicit-mobile-paired-desktop-selection.zh.md)。IndexedDB 配对存储仍是可注入的浏览器测试 adapter，打包入口无法选择它。
...
原生 shell 声明相机权限，并通过 WebView file input 使用操作系统 document picker。`@capacitor/app` 拥有前后台 lifecycle。完整的一次性配对邀请只会通过应用内 QR 扫描器或显式粘贴进入。两个原生工程都不登记配对用自定义 URL scheme，因为其他已安装应用可以声明同一 scheme 并拦截邀请。OAuth 凭据与 Account callback 值仍使用受运营 HTTPS 系统浏览器链路。iOS 17 或更高版本的 Debug Simulator build 可以把运营者提供的 HTTP CONNECT 代理仅应用于自身 Capacitor `WKWebsiteDataStore`；编译期 Debug 与 Simulator guard 会从 Release 与实体设备中移除该 adapter，因此验收不需要受监督设备配置或整机代理。

`mobile-release` Environment 提供唯一的 version/build 身份、生产公共配置、Android release keystore 与 Apple 上传凭据。Secret 值只进入消费它的 build 或 upload step；checkout、工具设置、依赖安装与 artifact 上传不会继承签名凭据。Android release build 只把 keystore 解码到 mode-0600 临时文件，校验 signed APK，并删除临时副本。iOS release build 在受保护的 self-hosted macOS ARM64 runner 上执行；缺失 Distribution identity，或 `Gestalt Mobile App Store com.gestalt.mobile` profile 过期或不匹配时会拒绝构建；archive 使用 team `MUX3KT56Q6`，export 使用显式 App Store profile mapping。TestFlight 上传是独立的 dispatch 控制步骤。Build artifact、profile、keystore 与 credential 都被忽略且绝不提交。
...
**只在发布 job 中生成 iOS 与 Android 工程。** 拒绝，因为签名设置、权限、自定义插件与原生升级行为将不再是可审查源码，generator drift 还可能在 code review 后改变 candidate。

**把完整配对邀请放入应用 URL scheme。** 拒绝，因为自定义 scheme 在 iOS 与 Android 上都不是独占的。QR 扫描与显式粘贴能让高熵邀请留在用户选择的应用内，且不需要域名关联基础设施。
