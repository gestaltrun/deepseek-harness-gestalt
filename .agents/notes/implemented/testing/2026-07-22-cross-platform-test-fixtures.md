...
The phone-runtime suite stages one `fakemobilecli.mjs` implementation through `stageFake`. POSIX hosts execute an extensionless shebang copy; Windows hosts execute a sibling `.cmd` launcher that delegates to the current Node executable and the shared module. Every scenario receives that helper's `executablePath`, so server startup, one-shot agent commands, signal escalation, stderr retention, and quiescent teardown continue through the production resolver and process owners without per-test platform branches.
