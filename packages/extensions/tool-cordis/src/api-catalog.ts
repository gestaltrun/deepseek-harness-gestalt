  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Fetch and publish one fresh grouped device listing. Online Android rows may carry `logicalDisplay` from `dumpsys display` `logicalFrame`.',
      ...
      },
      {
        ...
        description: 'Execute one semantic tap, swipe, text, or button action. Capture-source `x`/`y` and `captureWidth`/`captureHeight` remain the decoded plane. Android capture-source taps and swipes scale both axes onto the current incarnation `logicalDisplay`; missing logical bounds or a capture plane that fails the uniform full-frame aspect assumption fail with `PHONE_PROTOCOL` before RPC. A dumpsys miss does not replace the incarnation. Android fresh-probe pixels pass through. iOS obtains cached portrait `device.info` bounds and projects every displayed endpoint through exact rotation; browser actions bind current capture identity and model actions use a bounded fresh MJPEG EXIF probe. Button and text stay independent of coordinate conversion. Physical handsets are valid targets; only ids absent from the latest published listing fail locally before any RPC.',
        ...
        throws: ['{@link PhoneDevicesError} with `PHONE_DEVICE_NOT_FOUND` for ids absent from the latest published listing, `PHONE_PROTOCOL` when an iOS `device.info` answer lacks a valid positive screen size or Android capture-source input lacks a current logical display that matches the uniform full-frame aspect assumption, and otherwise per the class-documented failure modes.'],
      },
      {
        signature: 'async startCapture(request: PhoneCaptureRequest): Promise<PhoneCaptureStream>',
        description: 'Open one `device.screencapture` stream. `h264` maps onto upstream `avc`; Android pre-reads and replays at most one bounded key-access-unit probe, then replaces an invalid, failed, timed-out, or landscape-logical-display source with the system `screenrecord` H264 stream (`--size` from `dumpsys display` `logicalFrame` when known). Other bodies remain unread. A generation or incarnation change after headers joins foreign body cancellation for at most `captureCleanupTimeoutMs`.',
