...
Manual `Desktop Release`, `Mobile Release`, `Platform Image`, and `Platform Deploy` dispatches remain recovery entry points. Recovery is an explicit typed mode and never inferred from the triggering event. Each prior-run recovery verifies the same repository, allowed workflow, completed run, successful named producer job, and unexpired candidate-named artifact through the GitHub API; a later lane may fail without invalidating bytes already produced successfully, and the run head may be a later coordinator commit. The downloaded candidate manifest and recomputed digests bind the exact promoted bytes to the requested candidate. Mobile can reuse a validated IPA to retry TestFlight and carries validated prior TestFlight evidence when no new upload is requested. Platform Image recovery validates candidate-bound full-digest metadata and never accepts a requested digest as proof. Platform publish-only recovery validates the named deployment artifact and its exact candidate, image, version, and deployment metadata before creating the Release. A requested GitHub Release, TestFlight upload, or Platform publication fails its direct workflow when the external channel fails; the Product Release coordinator still aggregates lane failures through its final manifest. Interrupted Platform deployment recovery continues to use the durable production phase and rolling rollback transaction.
