...
部署会为每台 ECS 提供不同的 `PLATFORM_RELAY_INSTANCE_ID`，并通过 `PLATFORM_RELAY_*` 变量提供正数的容量、确认等待、directory TTL、heartbeat timeout、ciphertext buffer、连接数、待投递数与 attach timeout。PostgreSQL 拥有一次性附件 capability digest、pairing id、私有 OSS object key、byte length、expiry、账号 quota reservation、独占 consume claim 与容量；私有 OSS bucket 保存密文字节，但不会收到端点密钥或明文。加法 schema 会读取旧 PostgreSQL 密文行，并在 OSS 元数据旁兼容写入新密文，使新旧实例在滚动替换期间保持相互可读。上传会在发布元数据 authority 前完成。HTTP consume 会在响应前 claim authority，在响应 body 写入失败后恢复尚未过期的 claim，并且跨实例只接纳一份响应。consume、revoke、expiry 与 pairing revocation 会先提交元数据删除和 quota-release 工作，再清理对象；配置的定期 sweep 会以有界并发排队批量删除，限定在 `PLATFORM_OSS_OBJECT_PREFIX` 的一天 lifecycle rule 则会回收直接清理失败的对象。部署 preflight 通过 ECS IMDSv2 读取临时凭证，保留 bucket 上无关的 lifecycle rule，并在替换运行中容器前以 fail-fast shell 语义校验或安装精确的 namespaced rule。每次读取持久行时都会校验 digest 长度、品牌化 pairing id、精确 object-key binding、有界 byte length、安全整数 expiry、claim 与 quota reservation，校验通过后该行才能进入产品 authority；OSS 下载会在缓冲前要求 authority 指定的 `Content-Length`，并在 stream 中持续执行该上限。附件 HTTP 会鉴别当前 Mobile Installation 与确切的已确认 pairing selector，再预留账号完整 blob 配额；selector 本身没有 authority。应用会在监听前校验完整配置，包括 `PLATFORM_REMOTE_ATTACHMENT_SWEEP_INTERVAL_MS` 与 `PLATFORM_REMOTE_ATTACHMENT_CLEANUP_CONCURRENCY`。
