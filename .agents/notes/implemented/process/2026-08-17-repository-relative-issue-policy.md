...
The Gestalt organization tracker keeps Project lifecycle projection disabled because its Project and GitHub App authorization are not configured. `projectOrganization` identifies `gestaltrun` for a future optional deployment; owner alignment alone does not activate lifecycle without the repository variable and credentials.
