...
The Slot registry exposes `renderSessionSlot()` and ui-renderer exposes `mountSession()` as framework entries for a declared non-root Session slot. The mount resolves one explicit Session standard-props bundle through the existing `UiSession` adapter and renders it in an independent React root without changing `sessions.list.current`. The existing declaration ledger, entry boundaries, stores, inject faces, and standard hook binding remain authoritative inside that tree. Session creation and history opening remain Controller responsibilities.
