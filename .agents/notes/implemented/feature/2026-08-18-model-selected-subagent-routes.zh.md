...
只有 Agent 作用域的实例记录了非空 Session 策略，且绑定的 subagent 提供方声明 `SubagentCapabilities.agentOptions` 时，`dsh-tool-subagent` 才公开可选的 `provider`、`model` 与 `reasoning_effort` 字段。全新顶层 Session 会记录两类授权的确定性并集：来自[用户授权的 subagent 模型路由](2026-08-24-user-authorized-subagent-model-routes.zh.md)的已启用用户授权，以及 `ctx.subagentRoutePreauthorization` 的当前快照；两类来源可独立启用。部署服务与静态 Provider 分属不同包，Settings owner 不持有部署默认值也不写入设置，Provider 贡献由 effect 管理。禁用的实例会省略并拒绝面向模型的选择。如果可选择路由的实例缺少该提供方能力，插件挂载会失败。
...
- 主委派工具实例默认关闭选择，为新 Session 提供 Plugins 页面精确路由 opt-in，并且只在持久策略存在的 Session 中注册 `list_subagent_models`；发现与生效路由选择都受该策略限制。
