...
云端项目的成员 Service Definition:一个项目将规范化后的 git remote 作为已验证属性绑定,以三种权限角色 `owner|admin|member` 承载成员,并支持项目自定义功能标签。角色只治理这一协作层面;它不从 Git 平台权限派生,Git 权限也不从它派生。
...
`createProject(actor, {name, remoteUrl})`(创建者成为创始 owner)· `invite` · `retractInvitation`(发起人或 owner)· `acceptInvitation` · `declineInvitation`(仅收件人;收件人身份保持私密,其他账户只会看到 `INVITATION_NOT_FOUND`)· `changeRole` · `setMemberTags` · `removeMember` · `roster` · `pendingInvitationsFor` · `projectByRemote` · `rosterVersion`。
