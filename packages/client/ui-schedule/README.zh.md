...
本包只读取独立的 `schedules` Session projection。它不在浏览器中折叠 `schedule/change`，也不从 transcript 或工具调用渲染推断提醒状态。Client 时钟根据 `scheduledAt` 推导等待中或待补跑展示；持久化的 `paused` 标志来自 projection。Escape 会关闭任务板并将焦点还给触发器，点击外部会关闭任务板，空 projection 会在入口消失前关闭任务板。任务板通过 portal 挂到 viewport，右边缘与触发器对齐，并在 viewport 内向左钳制，因此在窄 Side Chat 中仍完整可见。
