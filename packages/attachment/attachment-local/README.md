...
`DSH_HOME` resolves through the shared path policy: explicit config, `$DSH_HOME`, then `~/.dsh`. Session logs contain only the reference and verified metadata, never this host path. `saveFile` uses the same atomic content-addressed publication for exact generic bytes up to the configured `maxFileBytes`; `readFile` verifies id, SHA-256, byte length, media type, and display name. `readImage` and `readFile` preserve caller cancellation.
