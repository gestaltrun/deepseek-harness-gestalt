...
`POST /phone/session` sends `{ deviceId, format: 'avc' }`. The live phase loads the signed `h264` URL. The devbar renders one H264 chip (`当前画面编码 H264 · 30 fps`); the MJPEG chip is gone. Host still signs both capture URLs; the client no longer requests MJPEG.
