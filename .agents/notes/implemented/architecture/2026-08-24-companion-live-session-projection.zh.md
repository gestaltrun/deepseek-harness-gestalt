...
协议测试固定加密的对端 offer 交换、应用数据前带 endpoint 方向的跨版本拒绝、严格的 `observe-session` 与 `session-live` codec、限制、移除字段与 revision 字段。Desktop 与 Mobile 单元测试固定 negotiation 超时与及时完成、Relay 对升级方向和容量错误的保留及共享 Mobile projection、已打开与隐藏 projection、过滤归档项后的位置、observation 切换和关闭竞态、stream 租约、合并、带 `hasMore` 的单个超大 entry 与累计 transcript 尾部、history 恢复、generation 替换、同 channel baseline 状态保留、teardown、重复 revision、有界 baseline 合并与共享组件 observation lifecycle。组装测试启动真实 Host Session persistence 和 HTTP/WebSocket stream，建立生产 Snow owner，在发送 revision 变化期间分页超过 20 个 Session，追加已经记录的 assistant chunk，并在保留已观察 conversation 的同时先交付 Workspace event baseline，再创建 Session。它还会在 Mobile 共享组件中观察真实 Workspace 重排、移除与 Session 归档。无密钥的 Mobile build-and-preview 捆绑快照会在 5173 和 5174 之外的端口展示同一可见变化，包括必须升级 Desktop 的状态。
