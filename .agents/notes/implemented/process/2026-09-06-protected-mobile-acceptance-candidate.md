...
The protected `mobile-release` Environment builds one release-signed Android APK through the existing packaging script. The workflow uploads the APK and a manifest as the repository-accessible `mobile-acceptance-candidate-<candidate_sha>` Actions artifact. The manifest binds the candidate commit, plan, version, build, APK digest, signing-certificate digest, and a digest of the operated Platform origin. It contains no credential or origin value. Candidate-build-only produces no iOS artifact, tag, Release, TestFlight upload, release acceptance, or publication evidence.
