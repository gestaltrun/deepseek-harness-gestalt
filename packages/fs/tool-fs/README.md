...
The published `@deepseek-ai/dsh-tool-fs/read-policy` subpath owns image-extension mapping, session-relative resolution options, missing-target observation, and regular-file admission for other model-facing readers. Consumers reuse it without loading the filesystem tool-registration plugin.
