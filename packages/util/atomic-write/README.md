...
- **Bounded Windows replacement retry** reuses the completed temp file when `rename` reports `EPERM`, `EACCES`, or `EBUSY`, allowing 310 milliseconds for a transient scanner or competing handle to release the target. Other errors and platforms fail immediately; exhaustion removes the temp file and rethrows the final error.
