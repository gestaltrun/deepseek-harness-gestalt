...
`set(session, name)` records a changed selection in a log-only `permissionPresets/preset` event, then calls each knob's setter only when its effective value changes. The selection event precedes the knob events and preserves user intent when presets share a bundle; a net-zero selection appends nothing. `current(events)` prefers a still-matching recorded selection, then the first matching table entry, and otherwise returns `custom`. A `source: 'delegation'` sandbox or approval event means the child is pinned to `'never'` approvals, so the select follows the inherited sandbox instead of reporting `custom`. Clients may display `custom` as the current value, but cannot select it.
