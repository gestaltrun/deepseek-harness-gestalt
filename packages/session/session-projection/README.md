...
- `ProjectionDefinition<K, S>` — `{ key, stateSchema, init(), apply(state, event), wire?, eventScope?, stateVersion }`: a synchronous state-driven computation unit. `wire` supplies `viewSchema` and `view`; omitting it makes the unit host-only. `eventScope: 'owned-suffix'` starts a fork at its immutable `SessionHeader.seedLength`; the default `full` includes inherited history.
