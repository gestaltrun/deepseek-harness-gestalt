...
Coverage tests retain the 30-second lane budget. The Sharp SVG rendered-text classifier has a 60-second test-local timeout because two hosted Windows executions reached the 30-second limit during native text rasterization. This fixture allowance does not change product runtime behavior or the timeout for other tests.
