...
19. **`src/client/state.ts` / `src/client/sidechat-restore.ts` / `src/client/subagent-detect.ts` / `src/client/builtins/tabs.tsx` / `src/client/api.ts` / `src/sidechat-core.ts` / `src/client/SideChatView.tsx` / `src/sidechat-routes.ts`** — 按 origin 隔离的标签条丢失后，Side Chat 标签页从持久化 Session 数据恢复。apply 生命周期内的投影订阅会恢复已发布、未归档的直属子线程，但不会替换活动标签。关闭操作会等待发布和持久化归档完成后再移除标签页；本地墓碑覆盖投影延迟。`?dsh-sidebar-reset` 会跳过恢复，共享 core helper 持有根线程身份。
