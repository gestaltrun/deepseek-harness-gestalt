...
[`apps/platform/tests/production-env.spec.ts`](../../../../apps/platform/tests/production-env.spec.ts) 钉住运行环境选择、缺失名称顺序、ECS instance id 校验、只列出名称不打印值的 CLI stderr、OIDC permission 与固定 action identity、加密暂存、云助手空结果轮询与终态结果校验、终止取消、独立恢复、全实例 finalization、candidate readiness、bridge 前置条件、contract 顺序、逐实例 rollback、SSH 名称缺席、`json-file` 轮转选项与 LoongCollector 注册。它也钉住 Platform Image 在 master 推送上构建但不推送到 GHCR。
