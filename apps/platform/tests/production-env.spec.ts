...
describe('Platform release workflows', () => {
  ...
  it('validates Environment production without applying ECS unless deploy is set', () => {
    ...
    expect(hostDeploySource).toContain('if ! docker pull "$collector_image"; then')
    expect(hostDeploySource).toContain('docker image inspect "$collector_image" >/dev/null')
    expect(hostDeploySource.indexOf('if ! docker pull "$collector_image"; then'))
      .toBeLessThan(hostDeploySource.indexOf('docker rm -f dsh-loongcollector'))
