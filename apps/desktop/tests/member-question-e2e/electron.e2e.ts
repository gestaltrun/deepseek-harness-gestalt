...
async function assertTransferredDocuments(name: InstallationName): Promise<void> {
  ...
  await getInstance(name).waitUntil(async () => {
    ...
    return text.includes('"documents"')
  }, { timeout: 10_000, timeoutMsg: `${name} did not persist transferred documents` })
  const state = JSON.parse(text) as {
    questions?: Array<{ documents?: Array<{ path?: string; bytes?: string }> }>
  }
  const documents = state.questions?.[0]?.documents
  if (documents === undefined || documents.length !== 3) throw new Error(`${name} did not persist all documents`)
  const decoded = new Map(documents.map(document => [
    document.path,
    Buffer.from(document.bytes ?? '', 'base64url').toString('utf8'),
  ]))
  expect(decoded.get('decision.md')).toContain('A1 authoritative guarded rollout')
  expect(decoded.get('preview.html')).toContain('A1 authoritative guarded preview')
  expect(decoded.get('notes.txt')).toContain('A1 authoritative plain-text acceptance material.')
  expect([...decoded.values()].every(value => Buffer.byteLength(value) > 32 * 1_024)).toBe(true)
}
