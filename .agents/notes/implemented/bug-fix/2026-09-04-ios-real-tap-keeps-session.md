...
Host `ioParams` converts iOS capture pixels with the cached `device.info.screenSize`, not scale alone. When scaled pixels overflow the sticky portrait bounds, it swaps width and height so WDA receives landscape logical points and never an x greater than the current logical width. Portrait taps stay on the unswapped size. Android still forwards capture pixels unchanged.
...
A landscape iOS real tap stays inside swapped WDA bounds and does not enter checking-agent. A left-side landscape tap whose scaled x still fits portrait width is forwarded unswapped; WDA already uses the live orientation for that in-bounds point. Picture and socket death still re-check a managed agent.

## Testing

`io.spec.ts` and `service.spec.ts` map 贝贝猫 landscape capture pixels (`2868×1320` at scale 3 from sticky `440×956`) onto swapped logical points and clamp overflow. `phone-connection.client.spec.ts` keeps an `agentManaged` iOS real or Android session live after a tap JSON-RPC error, and still takes device-offline and unauthorized arms.

## Related

Android landscape H264 box swap remains [the VideoFrame rotation note](2026-09-05-android-h264-videoframe-rotation.md).
