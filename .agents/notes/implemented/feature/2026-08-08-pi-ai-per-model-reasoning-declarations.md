...
`PiAiModelProfile.defaultReasoningLevel` selects one model's default independently of the route-level `reasoning` value. Resolution accepts it only when the exact materialized model offers that level, records it beside the route catalog, and gives it precedence when `resolveModelInfo` describes that model. The map stays outside pi-ai's `Model` object because it is a Harness selection default rather than a provider capability or wire spelling.
...
- **Letting the sidecar choose one route-level default.** Rejected because one Composite route can expose accounts whose upstream models have different defaults; collapsing them would make at least one model's selector lie or omit known metadata.
...
- Gateway-driven profiles can preserve each model's upstream default while retaining the route-level value as the fallback for models that do not declare one.
