...
The deployment supplies a distinct `PLATFORM_RELAY_INSTANCE_ID` per ECS host plus positive capacity, acknowledgement, directory TTL, heartbeat timeout, ciphertext buffer, connection, pending-delivery, and attach-timeout values through the `PLATFORM_RELAY_*` variables. PostgreSQL also owns the shared encrypted attachment table: Platform instances transact one-time capability digests, ciphertext, pairing ids, expiry, and capacity without receiving endpoint keys or plaintext. Attachment HTTP authenticates the current Mobile Installation and exact confirmed pairing selector; the selector alone has no authority. The application validates the complete bundle before listening.
