...
export async function apply(_ctx: Context, config: LocalCompanionListenConfig): Promise<void> {
  ...
  try {
    ...
    const desktopLifecycle = new RemoteRelayEndpointController({
      endpoint: 'desktop',
      route: async () => enabled.relay as NonNullable<typeof enabled.relay>,
...
function createInstallation(
  ...
  return kind === 'mobile'
    ? new PlatformAccountInstallation({
      ...common,
      installationKind: 'mobile',
      presentation: { name: 'Local phone', platform: 'ios' },
    })
    : new PlatformAccountInstallation({ ...common, installationKind: 'desktop' })
}
