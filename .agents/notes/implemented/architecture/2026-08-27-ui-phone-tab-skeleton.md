...
The fleet listing rides the shipped `PhoneListingSource` ([the listing-route note](../feature/2026-08-28-phone-device-listing-route.md)); `Config.enabled` remains the composition default the picker body pins. In-place device occupation is shipped in [the single-tab reversal](../feature/2026-08-29-ui-phone-single-tab-h264.md) and uses this registration path's `single: true` descriptor. The badge dot styling lands only when better-sidebar extends its pill contract. The hosting-root yield keeps the shared test invariant host deterministic for suites that provide their own `betterSidebar`; a regression that drops the registration in any other environment fails the companion at activation (`the "phone" tab is missing after the plugin fiber activated`). The Plugins-tab wizard card and the Host `ui-phone` namespace live in [the settings-wizard note](2026-08-28-ui-phone-settings-wizard-card.md).
