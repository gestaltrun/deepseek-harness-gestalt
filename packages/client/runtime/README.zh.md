...
`ISessions.stageProvisional()` 插入一条由功能持有、仅存在于客户端的 Session 摘要，使标准 Session slot 能在持久化 Session 出现前完成渲染。列表刷新会保留该行，`openForRender()` 会跳过 Host 历史请求，释放暂存会移除该行。匹配的 `host/session-added` 发布会把同一 id 原子升级为普通持久化行；之后临时身份的 disposer 不会删除已发布 Session。
