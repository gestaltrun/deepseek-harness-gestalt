...
通过下述分阶段序列，将运营 Platform 切换到 `https://www.beikejiedeliulangmao.top` 并支持裸域（`beikejiedeliulangmao.top`）。用户已确认以 www 为规范 origin，且已同时授权域名部署与正式的 Desktop/Mobile 重新发布；这些工作无需每步重新征求整体批准。拓扑决策已由 [issue #608](https://github.com/gestaltrun/deepseek-harness-gestalt/issues/608#issuecomment-5553279192) 中持久化的用户批准关闭：两台按月 `ecs.e-c1m2.large`（2 vCPU、4 GiB）实例，各带 40 GiB ESSD PL0 系统盘，首月总额上限 135.04，无年度承诺、无默认自动续费，且在网络、部署与回滚前置条件就绪前不采购——两个后端节点就此固定，不再处于评估开放状态。RDS 与 Redis 保留数据与实例，计费变更推迟到后续具体报价与批准。仍需基于各自具体证据等待显式批准的是：ALB Basic 迁移（本说明所处状态下仍为评估、未获批）、任何 RDS/Redis 计费变更、任何破坏性资源释放，以及超出已确认形态的任何高可用性取舍。本说明与其 PR 不执行任何变更：各阶段是授权执行遵循的持久计划，且在获批操作开始之前所有云上工作保持只读。
...
- 已选拓扑（用户选择；ECS 采购在支出上限内获批）：两台按月订阅的 `ecs.e-c1m2.large`（2 vCPU、4 GiB）ECS 实例——官方 `DescribePrice` 将展示的 47.52 计算价与 40 GiB ESSD PL0 系统盘的 20 元对账为每台 67.52、两台 135.04——仅按月计费，无年度承诺，无默认自动续费。RDS 与 Redis 的实例与数据保留，按月计费优化作为单独报价并批准的变更；负载均衡优化仍在评估且必须保留两个后端节点；单节点经评估未选择。两台实例及其系统盘的首月总额不得超过 135.04：若最终结账超出该上限，执行停止并询问。采购仅在网络、部署与回滚前置条件就绪后进行；旧 ECS 释放在新节点通过验收且释放授权得到解决之前保持推迟，并接受已披露的短暂重叠按量付费。
...
Stage 0.5 —— 成本方案验证（只读，然后有界采购）。ECS 选择已确定，其采购在上限内获批：两台按月 `ecs.e-c1m2.large`（2 vCPU、4 GiB）实例，`DescribePrice` 报价每台 67.52（47.52 计算 + 40 GiB ESSD PL0 系统盘 20），两台 135.04，实例加系统盘首月硬上限 135.04，无年度承诺、无默认自动续费，最终结账超出上限即停止并询问。采购仅在网络、部署与回滚前置条件就绪后进行，旧 ECS 释放在新节点通过验收且释放授权解决之前保持推迟，接受已披露的短暂重叠按量付费。本阶段仍以只读方式验证：2 vCPU/4 GiB 形态相对观测负载的利用率匹配；负载均衡选项——ALB Basic 需要新建实例切换而非原地 Standard 降配，其基础成本为每月 35.28 且不含 LCU 与 EIP（此前 27 的数字有误），在 Basic 的确切功能与策略支持及实例+LCU+EIP 的经常性成本经验证且用户确认之前，不对 Basic 做架构选择；当前监听器使用 SYSTEM 预设 `tls_cipher_policy_1_2_strict_with_1_3`，Basic 的自定义策略限制须精确验证，而非假设必须失去严格 TLS；以及 RDS/Redis 按月计费优化须凭具体报价获批，数据与实例保留。账单报告的分歧已由带时间戳的更正报告在方向上解决——2026-09-05T16:04Z 时余额 +86.52 且 ALB 无财务锁定——取代此前 FinancialLocked/−13.48 的读数；既定规则仍然成立：任何变更前立即进行当前的带时间戳复核，因为账户状态是时敏输入而非既定事实。两个后端节点由用户选择固定。
...
**把当前双实例/ALB 架构视为固定。** 已由持久化的用户批准解决而非保持开放：成本诉求打开了评估，单节点替代方案经评估未被选择，用户将拓扑固定为两台按月 `ecs.e-c1m2.large` 实例并保留 RDS/Redis——ECS 采购本身已在 135.04 首月上限内获批，不再处于待定状态。无条件的数据库/身份/数据保留与余下的显式闸门（ALB Basic 决策、RDS/Redis 报价、破坏性释放）仍是制衡。
...
- 成本方案在变更前经只读验证并记录：`DescribePrice` 报价的 ECS 对每台 67.52（总计 135.04，计算 + 40 GiB ESSD PL0 系统盘）及其 135.04 首月上限，无年度承诺、无默认自动续费；2 vCPU/4 GiB 形态的利用率匹配；ALB Basic 选项在任何选择之前先经验证——确切的功能与 `tls_cipher_policy_1_2_strict_with_1_3` 策略支持及实例+LCU+EIP 经常性成本，记录其不含 LCU 与 EIP 的每月 35.28 基础成本与新建实例切换要求；以及凭具体报价的 RDS/Redis 计费优化。所选双 `ecs.e-c1m2.large` 拓扑连同选定它的用户决策与支出上限一并记录，且任何变更前立即进行当前的带时间戳账单复核（2026-09-05T16:04Z 的更正报告显示余额 +86.52 且 ALB 无锁定）。
...
- ALB Basic 路径带有切换、验证与 TLS 策略未知数：它需要新建实例而非原地降配，其每月 35.28 基础成本不含 LCU 与 EIP，且是否支持当前 SYSTEM 预设提供的严格 TLS 姿态是待精确验证的问题——本计划不记录任何“Basic 必须失去严格 TLS”的假设，在该验证与用户确认落地之前不做架构选择。推迟的旧 ECS 释放及其重叠按量付费窗口是安全排序带来的已披露、有界的额外成本。
