...
- `ProjectionDefinition<K, S>`——`{ key, stateSchema, init(), apply(state, event), wire?, eventScope?, stateVersion }`：同步的状态驱动计算单元。`wire` 提供 `viewSchema` 与 `view`；省略它即为 host-only 单元。`eventScope: 'owned-suffix'` 从 fork 不可变的 `SessionHeader.seedLength` 开始；默认的 `full` 包含继承历史。
