...
`DONE` means a function is on `master`, though it may be newer than the latest installer. `DOING` means the product area has an active delivery. `TODO` means the work is on the product plan. The table is the README's only product-status inventory. Each row contains at most one product recording so the walkthrough reads vertically.
