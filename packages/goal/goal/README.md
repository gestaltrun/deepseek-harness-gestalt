...
Activation is never persisted. A fresh cache and every `agent/session-start` edge disarm it even when replay finds an active durable phase. A continuation driver also calls `disarm()` before unload or after durability uncertainty. Session resume, a seeded automation child, and driver replacement therefore retain the objective, phase, revisions, and admitted-round count without initiating work; a later explicit resume mutation must arm continuation.

`clearInherited(agent)` is the fork-boundary verb. A product fork that seeds a child session with a parent log prefix calls it once at creation: when the child's current goal came entirely from the seed prefix and the child has appended no goal mutation of its own, it commits one clear tombstone so the forked thread starts goalless. A child that already mutated its goal — including an earlier sweep's tombstone — keeps it. The durable tombstone makes the sweep idempotent: replay and later resumes read it and never re-run the sweep.
