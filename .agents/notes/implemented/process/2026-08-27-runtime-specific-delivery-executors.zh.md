...
在正常路径上，每个执行器只接收一个 ticket、分支和隔离 worktree。失去 worktree 隔离能力是唯一例外：这会触发共享 checkout 兜底，此时根任务仍是唯一的串行执行器，并会报告隔离能力降低。产品塑形原型保存在独立的已推送分支中，作为规划输入；实现 ticket 会改造这些代码，而不是直接合并原型分支。只有 Codex 运行时会归档 Codex 任务；精确验证 worktree 和分支仍是所有运行时共用的清理行为。
