...
describe('PostgresPersonalPairingAuthorityStore', () => {
  ...
  it('rejects unversioned and explicit unknown transaction formats before mutation', async () => {
    const pool = createMemoryPlatformSqlPool()
    const store = new PostgresPersonalPairingAuthorityStore('gestalt', pool)
    await store.migrate()
    await pool.query(
      `INSERT INTO remote_access_pairing_transactions (database_identity, state)
       VALUES ($1, $2::jsonb)`,
      ['gestalt', JSON.stringify(unversionedPairingTransactionDocument())],
    )

    const unversionedOperation = vi.fn()
    await expect(store.runPairingTransaction(unversionedOperation)).rejects.toThrow(/formatVersion/)
    expect(unversionedOperation).not.toHaveBeenCalled()
...
function unversionedPairingTransactionDocument(): unknown {
  const document = encodePairingTransactionState(emptyPairingTransactionState()) as Record<string, unknown>
