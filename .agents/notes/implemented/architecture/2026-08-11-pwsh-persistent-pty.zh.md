...
一个 backend、两种方言：`shellDialect: 'bash' | 'pwsh'`（默认 `'bash'`，存量部署逐字节不变）。有效 `shellPath`/`shellArgs` 按方言解析（bash `/bin/bash --noprofile --norc -i`；pwsh 经共享的 `dsh-pwsh-local` 解析器取 `-NoLogo -NoProfile`，保留交互宿主供子 REPL）。子环境去掉 bash 专属 `PS1`/`PROMPT_COMMAND` 标记并为 pwsh 加 `NO_COLOR`。pwsh 无法从环境安装提示符。在 Linux PTY 上，于 `PS …>` 之后交互写入不会执行，而 `-NoExit -File` 虽能打出标记但后续发送会被丢掉，因此 spawn 把 `HOME` 指到隔离 profile（钉住 UTF-8、定义 prompt 函数，并跑 stdin `ReadLine` 循环，`exit` 之后由 `finally` 结束进程），再等待 `Write-Output` 打出 `__DSH_PWSH_SETUP_DONE__` 以及 `dsh> `，且该 HOME 保留到会话关闭。提交的 pwsh 行在 Unix 上用 LF、在 Windows 上用 CR，因为 Linux 上的 PSReadLine 把 CR 当成光标回行首，CRLF 仍会先回行，setup 会回显而不执行。setup 源码在运行时拼接该提示符与标记，因为若源码本身嵌入 `dsh> ` 或该标记，PTY 回显会在函数运行前看起来像已就绪。Linux PTY 常常不再打印 `dsh> `；该标记才是 spawn 就绪信号。因为横幅到 setup 的间隙可能超过静默上限，所以会继续 follow-up 直到该标记可见。`session_exit`、单次 send 的 `timeout` 或 spawn 墙 `timeoutMs` 拒绝 spawn。[File/Session CI 修复](../bug-fix/2026-08-21-file-session-ci-reds.zh.md) 拥有该标记等待。两种方言发出相同的 BEL 终结 OSC `133;D;` 标记，因此 sanitizer、`PROMPT_MARKER_PREFIX`、`CONTROLLED_PROMPT` 与精确尾部就绪逻辑原样复用——标记仍只是就绪信号、载荷不被消费，与 bash 路径完全一致，且没有新增模型通知通道（与当前实现对齐；延后的 BEL 事件通道保持延后）。
