...
`@deepseek-ai/dsh-api-session-controller` owns the Client provisional identity lifecycle on `ctx.sessions`. `binding(id)` stays a render-safe lookup: it may mint a local scope for an eligible id, and it never opens Host history or refreshes a catalog. `stageProvisional()` extends controller-owned list eligibility with one caller-supplied blank subagent row, mints the ordinary `SessionBinding`, and leaves `list.current` unchanged. Duplicate staging of a still-staged or already listed identity fails loud; there is no shared second owner. A Host list refresh keeps unpublished provisional rows and, when the Host baseline already lists the id, publishes it in place while preserving the same binding. `openForRender(id)` is the explicit-render Host I/O: it skips history while the identity is provisional, and after Host publication it opens that Session's history and refreshes its subagent catalog without selecting it. SessionManager disposal marks itself first, aborts every catalog AbortController, ignores late catalog and list writes, does not start trailing catalog refreshes, awaits those catalog promises, then rebuilds the list snapshot without notifying. After SessionManager dispose, `refreshList()`, `handleConnected()`, catalog refresh, list mutations, and control/queue/projection sinks no-op. After ClientSessions root disposal, command methods (`open`, `openSubagent`, `clear`, `search`, `create`, `fork`, `refresh`, `refreshSubagents`, `setSubagentCatalogOpen`, `stageProvisional`, `resolveAgentScope`) throw `sessions.<op>: ClientSessions is disposed` before Host I/O or mutation. ClientSessions owns each in-flight `search`/`create`/`fork`/`refresh`/`refreshSubagents` before next-microtask Remote execution. Same-stack disposal suppresses unstarted Remote calls; started commands settle before the final list projection. Search is aborted; create/fork have no generated AbortSignal, so disposal awaits them without post-dispose binding or rename. `binding()`/`scope()`/`sessionOf()` do not rematerialize, `openForRender()` no-ops, and a racing provisional disposer is inert. A catalog refresh scheduled onto a microtask rechecks inflight identity before `subagents.list`. `refreshSubagents` passes the controller signal and writes only while the same inflight identity is live. Selected-path catalog membership updates are unchanged. An unknown identity is a no-op, matching the former explicit-render open. Publication deletes the provisional marker and reuses the same manager Session, scope, and binding. The returned disposer removes an unpublished row and its Agent scope exactly once, including before the first successful Host list baseline, and no-ops after publication or a prior release. Provisional-owner list mutations are tagged separately from ordinary upserts. During an in-flight list pull, those tagged upserts and removes do not replace or delete a Host row that publishes the same id; ordinary create upserts still replay. Host fields remain the publication.
