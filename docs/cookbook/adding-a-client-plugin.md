...
2. Register the package in `tsconfig.client.json`, add its `dsh.client` row to `packages/bundle/web-app/cordis.patch.yml`, and declare it in `packages/bundle/web-app/package.json`. Each location owns a distinct compile, Loader, or installation relationship.
