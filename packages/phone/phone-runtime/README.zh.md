...
- `io(request, signal?)` — 对应上游 `device.io.tap` / `gesture` / `text` / `button`。此 API 的 tap 与 gesture 坐标使用采集画面像素；iOS 会读取并缓存官方 `device.info.screenSize.scale`，再把像素换算成 XCTest 逻辑点后转发；Android 保持逐像素传递。`phoneSwipeActions`（以及 `@deepseek-ai/dsh-phone-runtime/swipe`）把 swipe 编成 [iOS swipe 笔记](../../../.agents/notes/implemented/bug-fix/2026-09-02-phone-ios-wda-swipe-gesture.zh.md) 拥有的 WDA 列表。真机是合法目标；仅最新清单中不存在的 id 在本包内以 `PHONE_DEVICE_NOT_FOUND` 失败。
...
- `screenshot(id, signal?)` — 通过 `mobilecli screenshot --format png` 捕获一张 PNG 静帧。真机是合法目标；仅最新清单中不存在的 id 在本包内以 `PHONE_DEVICE_NOT_FOUND` 失败。实况 MJPEG/H264 采集仍由 `startCapture` 负责。
