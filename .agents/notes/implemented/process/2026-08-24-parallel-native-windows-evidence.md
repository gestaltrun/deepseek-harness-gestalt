...
The coverage job runs eight single-worker shard processes concurrently on the repository's hosted eight-core Windows pool and on the existing failover pool. Its exempt-heavy gate shares a two-gate top-level budget with partitioned coverage. Build/runtime and static portability retain standard hosted runners. Each job has its own immutable install and 20-minute timeout, so setup duplication buys independent clocks without sharing mutable build output.
