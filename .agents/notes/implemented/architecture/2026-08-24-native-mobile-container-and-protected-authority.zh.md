...
已鉴权 Account 与 Personal Pairing 会选择一个账号隔离的 Companion Cache 数据库。由配对所有的 attachment material 经过 HKDF-SHA-256 派生的密钥，会使用配对专属 AES-GCM additional data 加密一份带版本的 Workspace、Session 与 transcript projection 快照。只有完整且通过校验的缓存 projection 才能成为 Remote Offline 只读展示；它不能创建已鉴权连接或启用 mutation。真实已鉴权 projection 会替换缓存展示，密封新的缓存快照，并继续以 Desktop 为权威。清除一个 Desktop 的缓存会删除其内容和回执，但不会删除受保护的配对权限。Session 创建、prompt、cancel、interaction settlement 与 attachment offer 都会在发送后保存 Operation Receipt。前台重连会向配对范围的 Desktop ledger 查询每个未知 operation id，应用其原始结果或明确的未提交状态，刷新展示，并且绝不重放 mutation。
