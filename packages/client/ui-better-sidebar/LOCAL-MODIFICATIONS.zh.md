...
13. **`src/client/SideChatView.tsx` / `src/client/index.tsx` / `src/client/api.ts` / `src/context-types.ts` / `src/sidechat-core.ts` / `src/sidechat-routes.ts`** — Side Chat 标签页只保留线程生命周期、切换与提升 chrome。它以子会话 id 通过 `uiRenderer.mountSession()` 挂载本仓 `conversation` slot，运行时准入适配器负责路由 prompt/cancel 并隐藏继承的 seed 前缀。快照自有的 transcript 轮询、消息渲染器、输入框、会话头信息路由、connection 镜像、相关 CSS 与未使用的本地化字符串均不存在。标准 queue/steer 输入形态分别到达 `agent.followup`/`agent.steer`；session scope 的会话头贡献按子会话 id 解析，workbench terminal 则保留自身 scope。
