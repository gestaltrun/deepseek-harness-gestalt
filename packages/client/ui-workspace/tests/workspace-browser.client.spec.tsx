...
describe('WorkspaceBrowser', () => {
  ...
  it('reveals a newly pending bound-Workspace session once and respects a later collapse', async () => {
    const quiet = summary('receiving', 1)
    const b = mount({
      useSessions: hook(sessionState([quiet])),
      useWorkspaces: hook(workspaceState([workspace('alpha', ['receiving'])])),
    })
    expect(screen.queryByText('receiving')).toBeNull()
