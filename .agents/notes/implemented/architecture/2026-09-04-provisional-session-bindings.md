...
Focused ClientSessions tests pin staging without selection, `binding()` without Host I/O, skipped Host history, list-refresh survival, in-flight refresh races, publication identity stability, release-once and post-publication no-op, duplicate-staging failure, published cold `openForRender()`, and HMR/plugin disposal.
