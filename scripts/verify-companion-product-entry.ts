...
function analyzeModule(
  ...
  collectDynamicImports(sourceFile, dependencies)
...
function collectDynamicImports(sourceFile: ts.SourceFile, dependencies: ModuleDependency[]): void {
  const visit = (node: ts.Node): void => {
    if (ts.isCallExpression(node) && node.expression.kind === ts.SyntaxKind.ImportKeyword) {
      const [specifier] = node.arguments
      if (specifier !== undefined && ts.isStringLiteral(specifier)) dependencies.push({ specifier: specifier.text })
    }
