...
The same sheets have stable `./styles/*.css` exports for bundled Web products that do not mount the dynamic theme plugin. Dark alias and Shiki overrides apply beneath any `data-ds-dark-theme` owner, so a phone-sized product root can scope its selected theme without mutating the Desktop-owned `body`.
