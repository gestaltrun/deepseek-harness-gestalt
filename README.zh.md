...
<div align="center">
  <p><strong>面向 agent（智能体）及其指挥者的持久工作空间。</strong></p>
  <p>
...
DeepSeek Gestalt 是建立在 [DeepSeek Harness](https://www.deepseek.com/harness/)（`dsh`）上的桌面端、Web 端与移动端工作空间。编码工作保存在持久会话中，你可以检查和恢复会话，把一部分工作拆进专门的 Side Chat，也可以在配对的手机上继续处理。

DeepSeek Harness 是由 [DeepSeek AI](https://deepseek.com) 开发的开源 agent harness（智能体框架）。它运行在 [Cordis](https://github.com/cordiverse/cordis) 之上，并将同一条规则贯彻到整个系统：**一切皆插件**。

## 愿景

agent 工作不应受限于一次提示、一个窗口或一台设备。Gestalt 正在构建一个连续工作空间，让模型、工具、文件、审批和客户端共享持久会话，而不是依赖不可见的聊天状态。

模型可见的每项输入都会记录进会话日志。需要人做出的决定保持明确。模型、工具、沙箱、浏览器、subagent、持久化和客户端等能力均可替换或扩展，不必修改某个特权核心。

## 实际运行效果

### 在完整仓库上下文中工作

Workbench 把对话、文件、仓库、终端和自由窗口放在一起。Better Sidebar 提供 Markdown 渲染、多仓库 Git 视图、文件导航和由 agent 操作的侧边栏动作。
...
Side Chat 是真正的子会话，不是临时面板。已经发布的 Side Chat 会保留历史记录和模型路由，在 Host 重启后恢复，并在关闭时归档。
...
Mobile Companion 通过 operated Platform 和加密 Relay 与桌面端配对。手机可以浏览由桌面端管理的会话、渲染其对话、处理待确认交互并提交工作，不会产生第二份真源。

<p align="center">
  <img src="https://raw.githubusercontent.com/gestaltrun/deepseek-harness-gestalt/3c711b7f0bc934d55f10dcdb9ee71e91850278f0/mobile-real-provider-98438f2.gif" alt="Mobile Companion 通过加密产品链继续桌面端管理的会话" width="360">
</p>

## 当前已经实现

- **持久会话：** 仅追加事件可重建模型历史、工具轨迹、回放、fork 和客户端投影；持久化会话还支持搜索。
- **agent 工作空间：** 文件编辑、Shell 与终端执行、LSP、浏览器控制、Web 搜索、计划、目标、后台任务和可继续的 subagent 均通过插件组合。
- **人的控制权：** 审批、提问、工具准入、设置和凭据引用不受模型控制。
- **多种客户端：** 浏览器 UI、Desktop Host、加密 Mobile Companion、CLI、ACP server、JSON-RPC client，以及 TypeScript 和 Python SDK 会按各自用途投影同一个 agent loop（智能体循环）。
- **可替换能力：** Profile 与 Bundle 通过配置选择模型提供方、工具、存储、沙箱、策略和 UI 贡献。

[架构文档](docs/architecture.zh.md)介绍插件树、会话生命周期和能力 seam。生成的[工具目录](docs/tool-catalog.zh.md)与[配置目录](docs/config-catalog.zh.md)列出当前运行时接口。
