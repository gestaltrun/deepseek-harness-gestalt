...
`DeepSeekHarness` keeps its lazily started runtime subprocess for reuse across calls. Use it as a context manager, as above, or call `close()` explicitly when finished. After the runtime acknowledges `shutdown`, `close()` waits up to `shutdown_timeout_seconds` for server-owned teardown, persistence, and normal process exit. A missing acknowledgement or exit timeout falls back to bounded terminate/kill cleanup, so closing never leaves the subprocess running indefinitely.
