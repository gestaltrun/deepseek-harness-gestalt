# Agent Note: 根 README 描绘 Gestalt 产品层
...
根 README 把 Gestalt 描述为桌面端与手机端工作空间，但没有准确说明它和 DeepSeek Harness 官方仓库的关系。读者无法看出哪些能力来自 DSH 基础、Gestalt 增加了哪些产品功能、集成了哪些社区插件，以及哪些工作正在交付或仍处于计划阶段。
...
**只把 Gestalt 描述成另一个 DSH 客户端。** 不予采纳：Desktop、Mobile、Workbench、发行、验收和社区集成共同构成产品层，不是一项客户端实现。

**列出每个包和工具。** 不予采纳：生成目录和源码负责完整清单。产品地图按用户可见的结果组织 Gestalt 增量。
