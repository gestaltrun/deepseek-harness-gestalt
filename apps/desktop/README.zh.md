...
主窗口只接受当前环回 Host 同源导航。包括 GitHub 授权在内的普通 HTTP 链接交给系统浏览器；其他来源和 scheme 不能替换 Session Surface，也不能创建另一个 Electron 窗口。Platform 账号签名密钥和令牌保存在 Electron userData 下的 `safeStorage` 加密文件中。Platform Account 与附件 HTTP 使用 Electron 可感知 session 的网络栈。每次 Relay WSS 建连时，Electron 都会在 attachment deadline 内解析有序系统代理列表；由于 Electron 的内嵌 TLS 运行时无法完成实际运行的 WSS 连接，一个捆绑的官方 Node helper 只负责通过各个不含凭据的 HTTP CONNECT 候选或 `DIRECT` 传输加密 Relay frame。helper 不接收 Platform Account、Snow、Session 或应用凭据状态，只继承证书与临时目录环境字段，在 IPC 处强制 Relay wire 上限，并由 Relay teardown 等待退出。候选建连跟随 Relay lifecycle 取消，且只对 allowlist connection failure 继续尝试。preload 只暴露当前状态与生命周期动词。Account 的 `beginLogin` 立即返回当前快照，因此 Settings 不会在 Host Account HTTP 期间被堵住。Account、Personal Pairing 与 updater 快照会同时投影到主 Session Surface 和当前 Settings overlay；已销毁或已替换的 overlay 不再接收后续事件。
...
ESM 主进程 bundle 会内联工作区代码，但把 Electron、`electron-updater` 和 CommonJS 运行时包 `https-proxy-agent` 与 `ws` 保留为外部依赖。Relay helper 是单独捆绑的 CommonJS extra resource，使官方 Node 运行时无需依赖打包应用的依赖图即可加载其 WebSocket 与代理依赖。packaged 冒烟测试会隔离 `DSH_HOME` 与 Electron userData；在 macOS 上保留已登录用户的 home，使 `safeStorage` 能访问 login Keychain。
