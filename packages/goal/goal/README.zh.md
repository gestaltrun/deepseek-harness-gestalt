...
`clearGoalFromForkSeed(seed)` 在产品级 fork 发布前准备种子。当连续的父日志前缀末尾存在当前目标时，它返回追加了一条 clear 墓碑的新种子；否则返回原种子。宿主把父前缀长度记录为 `header.seedLength`，因此即使创建流程原子地接纳完整序列，该墓碑仍属于子会话。未调用此 helper 的自动化提供方保留种子中的目标状态。
