...
- `sessionFor(id, agentManaged)` — IO 升级路径以及签名的 `mjpeg` 与 `h264` URL，查询 token 在 `tokenTtlMs` 后过期；`agentManaged` 标记 iOS 真机会话。
- `POST /phone/session` — 为最新清单中存在的设备签发这些 URL；先执行 `/api` 信任栅栏。iOS 真机会先运行 `agentStatus`：未安装时返回 `PHONE_AGENT_MISSING`；成功会话携带 `agentManaged: true`，使后发画面失败能够复检 agent。
