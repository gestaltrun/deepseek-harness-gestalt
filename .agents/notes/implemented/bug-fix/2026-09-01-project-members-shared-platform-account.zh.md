...
手机配对与 Project Members 观察并修改同一份当前安装 Platform Account session。退出登录或切换 Account 会立即门控 Project Membership，登录会恢复仍在打开的设置界面与邀请轮询，Account 凭据不会进入 renderer state。聚焦 UI 与组合测试覆盖全部投影状态、稳定 snapshot 身份、登录导航、查询与邀请抑制、Account 切换和恢复；运行态验收仍需在真实 Platform 与 Electron 界面上使用两个真实 GitHub 账号。
