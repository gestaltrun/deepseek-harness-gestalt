...
Vault tests persist two independent grants, names, keys, reconnect records, and an explicit selection through both IndexedDB and native protected-storage adapters; they also prove duplicate-id rejection, selected-only release, and account isolation. Controller tests prove old projection and Relay release before new grant activation, durable offline and capacity-shed selection, stale activation rejection, selected-only Platform revocation, preservation of the other pairing, and no implicit selection after removal. The authenticated receiver test proves that only the encrypted Desktop projection supplies the retained name. Separate English and Chinese bundled Mobile snapshots render the localized selected and unselected Desktop controls before executing the shared Session and conversation components.
