...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` are the one shared scrub definition: ambient credential-shaped and `DSH_*` names are dropped. `childEnv()` then applies explicit entries with case-insensitive replacement on Windows, so `PATH` cannot coexist ambiguously with an inherited `Path`. The local ordinary and terminal spawns both apply these helpers; SDK-managed transports that own their spawn may import them directly.
...
- **SDK-managed spawns remain outside** — an SDK transport that owns its internal spawn cannot route that call through this service; it can still import `childEnv` so scrubbing and platform-specific environment-name semantics stay single-sourced.
