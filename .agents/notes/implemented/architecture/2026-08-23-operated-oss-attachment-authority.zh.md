...
加法迁移让 `remote_attachment_blobs` 保持可读，增加共享 claim token，并记录部署级 `legacy` → `draining` → `bridge` → `oss` authority phase。PostgreSQL ciphertext publish 会先在 `remote_attachment_postgres_publish_intents` 中提交 `recovery` owner，再用独立事务把完全匹配的行推进为 `intent`，最后在持有每个 digest 的 session advisory lock 时原子地用 blob 替换 intent。intent 推进回滚或结果无法读取时，recovery 行继续拥有恢复责任。容量计数覆盖 blob 与两个 publish-row stage 的并集。重启 reconciler 会跳过 active publisher，把孤立 recovery 或 intent 行转换成持久 quota-release 工作，并让已提交 blob 由正常的有界 expiry cleanup 处理。账号 quota admission 本身是 store 接收前已提交的持久过期 lease。实际运行 lease 是已配置 capability lifetime 的两倍，每个 attachment store 都会拒绝早于预定 blob expiry 结束的 lease。因此 recovery prewrite 回滚、结果不可读且主动 release 也失败时，只会在 lease expiry 前保留有界 quota overcount，不会造成 active blob undercount。authority 为 `legacy` 时，当前 PostgreSQL 进程会保留 fixed-base 的 inspect／write／revoke 行为；每个已准入 response 都会在 dedicated client 上持有 PostgreSQL session shared advisory lock，直到 complete 或 abandon。cutover 会先提交 `draining` 再等待 exclusive lock，因此新请求无法加入，连接丢失也会自动释放所有权。OSS cutover 会在删除未 claim 重复行前完整解析并比对 object／legacy digest、key、pairing id、byte length、expiry、quota reservation 与 claim state；legacy-only 行保持可读直到 consume 或 expiry。OSS publish finalization 与 cutover 使用同一 capacity advisory lock，因此 OSS authority 生效后不会再提交兼容 ciphertext。

每个请求都由当前 Mobile Installation 与已确认 pairing 鉴权。Platform launch 会把该 request authenticator，以及与 `PersonalPairingProvider` 一同返回的账号已鉴权 quota closure，直接捕获进 attachment HTTP plugin；这两种 authority 都不会发布为 Cordis service，也不会暴露为 public provider method。上传要求正数且精确的 `Content-Length`，在读取 request body 前预留账号完整 blob 配额，并在读取被拒绝或失败后释放准入。元数据会保留不透明的品牌化 reservation id，直到 consume、revoke、expiry、pairing revocation 或 publish-intent recovery 时释放。定期 PostgreSQL sweep 只删除 authority 从本轮 candidate set 中明确返回的 pairing id，记录持久 quota-release 工作，并以配置的并发数排队删除 OSS。publish 不等待已排队的 object deletion；dispose 会等待 active sweep 与 cleanup worker。OSS lifecycle 仍是对象删除失败的兜底。持久读取会在缓冲单个预分配 object buffer 前拒绝格式错误的 digest、pairing id、object key、length、expiry、claim 与 reservation id；所有 OSS stream 失败都会销毁 stream。
...
`oss-client.spec.ts` 固定 IMDSv2、临时凭证校验、私有 object header、单 buffer 有界读取、stream 销毁、lifecycle 保留、精确 rule 幂等性与缺少 lifecycle 时的创建。`oss-attachment-store.spec.ts` 会拒绝格式错误的持久行，在清理失败时保留 capacity 错误，并在 commit 结果不明时保留 object。Attachment HTTP composition coverage 会验证被捕获的 authenticator、Loader 挂载的 route fiber、dispose 时移除 route，以及固定诊断中不存在含 secret 的 exception 内容。`product-entry-durable.spec.ts` 使用临时 PostgreSQL 证明 fixed-base inspect-write-revoke 重叠、bridge-to-OSS claim 独占、candidate-only revocation、quiescent dispose、两实例容量计数、recovery prewrite 回滚且结果不可读并且主动 cleanup 失败后的 lease expiry，以及 intent 推进回滚且结果不可读、仅 intent crash、主事务回滚或主事务已提交但 readback 失败后的 PostgreSQL publish recovery；`production-env.spec.ts` 固定两种部署 mode、candidate readiness、contract 顺序与逐主机 rollback。
