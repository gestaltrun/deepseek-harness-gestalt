...
| `ctx.memberQuestionSender` | `seam` | [`member-question-sender`](../packages/interaction/member-question-sender) | [`member-question-sender`](../packages/interaction/member-question-sender) | [`tool-ask-user`](../packages/interaction/tool-ask-user) | - | 经注入的 port 编码并投递 Companion member-question 操作；同一 port 原子保留并重放首个终态 claim，对端凭证来自 B 侧项目对等授权检索。 |
