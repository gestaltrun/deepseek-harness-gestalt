...
Each documentation build owns its complete resolved `outDir` as disposable output. `website/build.ts` removes that exact directory after VitePress resolves its configuration and before the current bundle or `public/` files are written; a link-shaped output path is unlinked without traversing its target. Raw projection does not remove files at `buildEnd`, so its collision check sees every file produced by the current build and still rejects a public file, page, or image at the same path.
