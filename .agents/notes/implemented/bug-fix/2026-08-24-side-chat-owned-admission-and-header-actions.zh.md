...
Side Chat 只有一棵标准对话组件树和一条功能自有准入路径。打开标签页在首次提交前不会改变拓扑；模型与 catalog 交互也不会发布 Session。临时状态下的权限变更使用父会话自有的命令路径，发布后的变更与直接父会话保持同步，queue 编辑与 steering 会到达在线 child inbox。紧凑头部只展示当前渲染 child 的下级；选择下级时外壳主 Session 与 Side Chat 标签页都保持不变，任务列表在窄侧栏中仍完整可见。新增的功能自有 Session 操作必须使用能够校验所寻址身份的归属方。
