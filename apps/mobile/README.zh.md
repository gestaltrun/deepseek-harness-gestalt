...
- 加密 projection cache 与清除缓存 UI 已接入。Operation Receipt 持久化已经存在，但产品 mutation sender 尚未让 prompt、cancel、interaction 与 attachment send 经由 `CompanionUncertainOperationSettlement`；assembled reconciliation 目前只覆盖 uncertain attachment status。
