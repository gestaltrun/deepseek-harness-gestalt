...
页脚同一行：左侧 `sidebar.settings`，右侧可选的 `sidebar.footer.action`（轨道上动作叠在 Settings 上方）。ui-settings 注册触发行和全屏设置页。Desktop 原生 overlay 文档只渲染 `sidebar.settings`。`sidebar.brand` 是一条 chain，回退为 HARNESS 字标。浏览器组合里 `sidebar.chrome.drag` 是空列表。
