...
Packaged Desktop starts signed-in Personal Pairing only after the Web Host is installed. Account sign-in and process resume use the same readiness predicate, so waking while initial or replacement Host startup is incomplete cannot restart Relay access. The pairing controller serializes start, deactivate, and replacement start across one lifecycle generation owner. It rechecks the captured Host predicate before and after refresh, stops a stale in-flight start as `host-unavailable`, and admits the replacement only after that cleanup. Host exit still retains a previously established Relay long enough to return typed Host failures; generation cancellation applies only to an in-flight startup that has not established current Host authority.
