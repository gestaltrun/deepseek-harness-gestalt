...
- `io(request, signal?)` — 执行一个封闭的语义 tap、swipe、text 或 button 动作。Android 把 tap 与 swipe 转发到 `device.io.tap` 和 `device.io.swipe`。iOS 旋转 0 度时使用相同方法；旋转 90/180/270 度时，运行时把显示采集点逆变换到竖屏事件坐标，swipe 通过 `device.io.swipe` 发送，tap 则作为零距离 `device.io.swipe` 发送。精确旋转来自当前 H264 帧受信任的 `captureRotation`、实况 MJPEG EXIF 观察，或模型独立调用触发的一次有界单飞 MJPEG 探针；绝不从宽高猜测未知旋转。观察与探针绑定当前运行时 generation 和设备。真机是合法目标；仅最新清单中不存在的 id 以 `PHONE_DEVICE_NOT_FOUND` 失败。
