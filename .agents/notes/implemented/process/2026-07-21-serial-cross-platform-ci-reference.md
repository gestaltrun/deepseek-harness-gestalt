...
[CI](../../../../.github/workflows/ci.yml) and [CI master](../../../../.github/workflows/ci-master.yml) give candidate, master-push, scheduled, and manual events complementary responsibilities. Candidate evidence runs the complete platform inventory on standard hosted capacity. Every master push runs bounded Linux and Windows standby smokes on the in-house `vm-backup` and `dsh-win-ci` pools; daily or `standby-exhaustive` dispatch runs the complete unsharded references there. The standard-hosted `serial / macos` remains disabled under `TODO(hosted-serial-ci)` until its portable capacity can be restored. Separate job definitions keep checkout, runtime setup, clean installation, and platform ownership visible instead of hiding operating systems behind a matrix or reusable workflow.
