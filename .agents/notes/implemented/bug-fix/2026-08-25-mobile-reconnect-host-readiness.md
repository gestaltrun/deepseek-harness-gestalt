...
The first physical Relay attachment also remained inside the controller's Settings serial operation. When a retained pairing's Mobile was offline, that attachment could keep waiting while Mobile Access toggle and invitation operations accumulated behind it, making the visible switch appear inert.
...
Packaged Desktop starts signed-in Personal Pairing only after the Web Host is installed. It schedules that lifecycle start without delaying `window.loadURL()`, so a retained Mobile Access state or slow Relay attachment cannot leave the Desktop window blank while the Host is already available. Account sign-in and process resume use the same readiness predicate, so waking while initial or replacement Host startup is incomplete cannot restart Relay access. The pairing controller serializes start, deactivate, and replacement start across one lifecycle generation owner. It rechecks the captured Host predicate before and after refresh, stops a stale in-flight start as `host-unavailable`, and admits the replacement only after that cleanup.

After synchronizing retained grants and publishing current Platform state, the controller starts physical Relay attachment as one tracked background attempt. Settings operations do not await first attachment, so state changes and invitation management remain usable while a paired Mobile is offline. A current-generation Relay start failure becomes a visible failed projection and a diagnostic; a stale attempt cannot mutate the replacement generation. Lifecycle stop preempts and drains the underlying Relay attempt. Host exit still retains a previously established Relay long enough to return typed Host failures; generation cancellation applies only to an in-flight startup that has not established current Host authority.
...
**Move only the toggle outside the Settings serial operation.** Rejected because invitation, confirmation, revocation, polling, and account-lifecycle work would still starve behind the same offline attachment, while concurrent mutations could race the controller's authoritative projection.
...
Desktop readiness coverage keeps pairing stopped for every incomplete Account/Host combination and delegates the current-generation predicate to the controller. Lifecycle coverage interleaves a slow old start, Host replacement stop, and new start, proves only the replacement owns Relay, and proves the Mobile Access mutation begins while an offline attachment remains unsettled. Mobile coverage rejects offline history submission and keeps one clock subscription across renders. A packaged Desktop with retained Mobile Access loads its Web Host window before the existing pairing finishes Relay startup. Desktop stop and replacement leave the Android opened conversation mounted, disable its composer while offline, restore it without a Host-unavailable error, and preserve the Account, pairing, keys, and cache across an in-place APK upgrade.
