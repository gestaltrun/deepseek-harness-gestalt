...
**把密文留在 PostgreSQL。** PostgreSQL 仍拥有单次 consume、容量、过期与撤销所需的跨实例事务，但[实际运行的存储决策](../architecture/2026-08-23-operated-oss-attachment-authority.md)会把大型密文字节交给私有 OSS；除滚动兼容行外，PostgreSQL 只保留紧凑 authority。
