...
`defaultReasoningLevel` selects the default independently for one model and must name a level that the resolved `reasoningEfforts` offers. It takes precedence over the route-level `reasoning` default for that model; omission keeps the route-level behavior. This lets one gateway publish models whose upstream defaults differ without widening any model's selectable levels.
