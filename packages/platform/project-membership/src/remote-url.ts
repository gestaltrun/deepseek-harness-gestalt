 ...
 * one canonical spelling of the workspace's git remote so `projectByRemote`
 * matches independently of how a caller spells the same checkout.
...
const invalidRemote = (input: string): ProjectMembershipError =>
  new ProjectMembershipError('INVALID_REMOTE_URL', `"${input}" is not a normalized git remote URL (use https://host/path or user@host:path)`)
 ...
 * the host and keep the user and path case-exact; every accepted spelling
 * drops one terminal `.git` repository suffix (case-insensitive) and trailing
 * slashes while keeping mid-path segments untouched. Anything else is
