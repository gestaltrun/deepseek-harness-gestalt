...
自托管的 Linux 与 Windows standby 池，以及被禁用的 `serial-macos` 任务仍然存在。拉取请求使用主路径和标准托管兼容性作业，其他大型运行器规格仅通过手动触发运行。

公司自有 Linux 池（`vm-backup` 标签：一台 64 核虚拟机，运行 6 个常驻的 systemd 管理运行器实例）会在每次 `master` push 后运行有界就绪 smoke，并按日或通过显式 dispatch 运行完整的非分片参考流程。它们属于 standby 证据，而非必需检查。实际切换机制已预先布线：必需 Linux 作业通过写入权限持有者可管理的仓库变量 `DSH_CI_FAILOVER_LINUX` 解析运行器池，因此故障响应就是设置一个变量并重跑——无需合并（合并本身会被正在失败的检查阻塞，形成死锁）（[切换手册](2026-07-26-ci-failover-runbook.zh.md)）。故障切换期间，`pull_request` 作业会带着 PR merge 引用自带的工作流定义到达这些运行器；信任边界是仓库成员资格（仓库为私有且禁用 fork，选择器排除 Dependabot），详见手册记录。
