...
phone-runtime 套件通过 `stageFake` 暂存唯一的 `fakemobilecli.mjs` 实现。POSIX 宿主执行无扩展名的 shebang 副本；Windows 宿主执行同目录的 `.cmd` launcher，由它转交给当前 Node 可执行文件与共享模块。所有场景都接收该 helper 的 `executablePath`，因此服务启动、一次性 agent 命令、信号升级、stderr 保留与完全停稳的资源清理继续经过生产解析器与进程持有者，无需在各测试内分散平台分支。
