...
4. Follow the [runtime-specific executor decision](../../notes/implemented/process/2026-08-27-runtime-specific-delivery-executors.md) for product-shaping prototypes. Do not draw UI in the coordinating session. Under Codex, create an independent Codex Worktree task with a short brief (question, host page, component library, forbidden chrome). Under DSH, use `subagent_fork` only when the brief is already written; otherwise dispatch a plain `subagent`. Keep prototype code on its own pushed worktree and branch. The prototype session follows [prototype](../prototype/SKILL.md): fuse interaction variants into the existing page, self-check headless through [dsh-desktop-test-instance](../dsh-desktop-test-instance/SKILL.md), then open a headed instance only to ask the user to review. Implementation tickets adapt that code instead of merging it verbatim; retire the prototype branch once its consuming tickets have landed.
