...
- [`product-workflows.spec.ts`](../../../../scripts/release/product-workflows.spec.ts) and the Desktop workflow tests pin CI validation, Product Release PR generation without publication, least-required reusable-workflow permissions, trusted executing revisions, exact candidate and version inputs, sequential named Desktop artifact downloads, named-artifact recovery, direct channel failure propagation, durable Mobile prereleases, Platform deploy and publish-only recovery, selected-lane invocation, and final manifest artifacts.
