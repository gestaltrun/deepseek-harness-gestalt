...
卡片控制器在每次启用态 publish 时经重入护栏触发 source 的一次性 `ensureDetected`——探测在 kick 内完成会同步 notify，重入的 publish 不得再次 kick（P17 递归环）——而非在构造时抢跑 redetect——构造时 scope 通常尚未水合，因此首次启用渲染即探测中视图，只有探测本身失败才落到 probe-failed 臂。
