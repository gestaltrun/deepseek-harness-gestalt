...
面向成员的提问通过扩展 `ask_user_question` 的参数完成，而不是再注册一个面向模型的工具。新工具会把同一套提问词汇拆成两个名字，迫使模型在兄弟工具之间选择，却仍要共用本地回答 JSON。因此现有 `ask_user_question` schema 增加 `to_project_member`、`background` 和 `references`。每次执行都只调用 `ctx.userQuestions.ask()`。路由执行添加最小 `memberRoute` 请求字段，并保留品牌化的 Project、Account 与 Companion Session identity。发送器以 `prepend` 在 Host 根上下文注册非 scoped waterfall answerer，因此会在已有 Remote 或 UI answerer 之前认领该请求；普通请求调用 `next()`，并按 Cordis 注册顺序继续传递。`background` 仅在路由模式必填，构建期按 T4 的 600 码点上限以 `BACKGROUND_REQUIRED` 或 `BACKGROUND_TOO_LONG` 拒绝。`references` 是本地与路由提问的常态参数；每个 `path` 必须存在于提问会话工作区内，每个 `reason` 至多 100 个码点，否则工具抛出 `REFERENCES_INVALID` 并指出失败项。
...
当 `send()` 被给予提问会话时，它会追加可忽略、仅写入日志的 `member-question/asked` 与 `member-question/outcome` 事件。这些记录已经作为工具调用与工具结果对模型可见，因此它们不是 surface 事件，也不会重新进入派生历史；较旧的 harness 可以跳过这些审计记录，而不改变重建出的模型历史。
...
**让工具直接调用发送器。** 否决：直接派发会绕过共享的 user-question waterfall，并让一个 Consumer 选择 Provider。最小 `memberRoute` 请求让所有提问继续走 `ctx.userQuestions.ask()`，同时由 Host 根上下文的发送器 answerer 拥有 Companion 投递。
...
**让 ask/outcome 审计记录保持 required-on-read。** 否决：工具调用与工具结果已经足以重建模型可见的交互。要求 reader 理解重复的审计词汇，会在跳过这些记录仍可保留模型历史时无谓拒绝日志。
