...
GitHub Actions uses Environment `production` only. [Platform Image](../../../../.github/workflows/platform-image.yml) builds on pull requests and matching master pushes, and pushes to GHCR only on `workflow_dispatch` with `inputs.push`. [Platform Deploy](../../../../.github/workflows/platform-deploy.yml) always validates the production and ECS names through [`production-env-cli.ts`](../../../../apps/platform/src/production-env-cli.ts) (`node --import tsx/esm`) and SSHes only when `inputs.deploy` is true. It starts and verifies a candidate on every host before the global contract, requires a live predecessor on every host, stops all predecessors before any replacement receives traffic, and restores only hosts with a renamed rollback container. Attachment storage uses two invocations: `postgres` establishes the atomic bridge, then `oss` requires every predecessor readiness response to report that bridge. The CLI entry is not bundled into `boot.mjs`.
