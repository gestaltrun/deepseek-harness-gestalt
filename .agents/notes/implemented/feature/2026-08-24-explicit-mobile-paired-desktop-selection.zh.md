...
账号隔离的受保护 pairing 文档使用版本 2。它原子保存一组有界独立 pairing 记录，以及一个可选 selected pairing id。每条完整记录拥有自己的 Personal Pairing id、Mobile Relay grant、Snow reconnect state、attachment key，以及只从已鉴权 `foreground-sync` 获得的可选 Desktop 名称。原生 version-1 文档或无 version 的 IndexedDB 文档会迁移每条 retained record，从各 record 的 pairing id 推导缺失的 Mobile grant selector，并按稳定 legacy 插入顺序选择最后一份完整 authority。迁移会在返回 authority 前持久保存 version 2。冲突 selector、其他版本、重复 Personal Pairing id、缺少完整权限的 selected id、畸形 secret 字节和超过 retained-pairing 上限的记录都会 fail closed。Account 选择会串行执行 load 与 save，把每份 secret 复制进 active scope，并清零转移来的 buffer。
...
迁移只接受仓库发布过的两种前序编码，并保留其中有界多 pairing 的插入顺序。选择最后一份完整 legacy authority 会保留 version 2 前的行为，同时不丢弃其他完整或 partial record。不支持或损坏的文档仍需移除保留的受保护值并重新配对。Android 卸载会删除这份应用数据；iOS Keychain 可能在卸载后继续存在，因此仅重装应用不能可靠移除不兼容文档。
