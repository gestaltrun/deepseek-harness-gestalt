...
Workspace 行的 ⋯ 菜单以**工作区设置**为首项。设置弹窗承载工作区升级区块：先用当前 Workspace 规范化后的 `origin` 创建云项目，然后是对已绑定项目的成员管理——每行名册的显示名、角色选择、职能标签编辑、在线点和移除——以及按 GitHub 登录名邀请和在设置重开时重新加载的权威可撤回待确认邀请行。该界面中的 `origin` 只读；非 Git Workspace、没有 `origin` 的 checkout 或不受支持的 remote 都不能创建云项目。所有操作都经由可选的 `projectMembership` gateway prop 路由，由 Desktop 组合适配 `@deepseek-ai/dsh-project-membership-client` 并走 `/v1/projects` 路由；没有该 gateway 的组合仍显示菜单项，但不渲染升级主体。Desktop 会以当前安装共用的 Platform Account 为每一次 membership 读取和 mutation 设置门控。退出登录的设置界面打开既有的手机配对 Settings 登录流程；同一个 Account source 报告 `signed-in` 后，界面继续恢复 Project。Workspace Settings 不拥有第二套登录流程或凭据。打开设置时会按真实 `origin` 解析当前 Account 已有的 Project，再在渲染名册前读取持久化的精确本地 binding。恢复得到的 Account id 会选择其权威名册行：admin 和 owner 获得邀请与变更控件，普通或无法识别的 member 只读名册，也不会发起仅邀请方可用的请求。部分完成的创建若缺少 binding 会被修复；若 binding 已指向另一个 checkout，则界面报告冲突，绝不会仅因两个 checkout 同源就迁移 binding。创建 Project 会在结果到达界面前持久化 founder 的 Account／Project／Workspace binding。15 秒一次的待确认邀请轮询驱动两步邀请向导：第一张卡展示邀请人、Project 与可信 bound remote，并提供接受／拒绝，随后是必选的本地工作区关联步骤。Host 以无 shell 方式读取每个 checkout 的 `origin`，把 remote 一致的工作区标注「同源推荐」、已知不同源标注「异源」，也可让用户选择父目录，把邀请 remote 克隆并注册为新 Workspace。接受操作先持久化精确 Account／Project／Workspace binding，再提交 Platform membership；取消不会提交两者。没有「暂不关联」，关闭向导不做任何决定，邀请回到待确认池中。
