...
The Gestalt organization tracker keeps Project lifecycle projection disabled because no Project identity or GitHub App authorization has been confirmed. The current `projectOrganization` is a disabled sentinel that intentionally differs from the repository owner, so accidental activation fails during owner validation before token creation. A future deployment replaces the sentinel only after confirming the target Project and credentials.
