...
Mobile Pairing and Project Members observe and mutate one current-installation Platform Account session. Signing out or switching Accounts gates Project Membership immediately, signing in resumes the pending settings view and invitation polling, and Account credentials remain outside renderer state. Focused UI and composition tests cover every projected state, stable snapshot identity, sign-in navigation, lookup and invitation suppression, Account switching, and recovery; operated acceptance still requires two real GitHub accounts against the real Platform and Electron surfaces.
