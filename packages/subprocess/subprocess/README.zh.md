...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` 是唯一一份共享的环境清理定义：环境中形似凭据的名称与 `DSH_*` 名称都会被丢弃。`childEnv()` 随后应用显式条目，并在 Windows 上按大小写不敏感语义替换名称，避免 `PATH` 与继承的 `Path` 同时存在。本地的普通 spawn 与终端 spawn 都应用这些 helper；拥有自身 spawn 的 SDK 管理传输可直接导入它们。
...
通过消费方间接影响（目前是 `dsh-tool-bash` 背后的 bash 执行器家族）；进程输出和生命周期的全部面向模型渲染均由消费方负责。
...
- **由 SDK 管理的 spawn 仍在服务之外**：拥有内部 spawn 的 SDK 传输无法把该调用路由到本服务；它仍可导入 `childEnv`，使清理与平台环境名称语义保持单一来源。
