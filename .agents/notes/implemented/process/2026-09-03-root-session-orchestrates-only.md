...
The root may read GitHub, the tracker, worker reports, and CI status; write a brief; create an empty specification branch and Draft pull request; and enqueue a merge once reported evidence passes. It does not land code, documentation, or environment edits in the coordinating checkout. [The spec-PR decision](2026-09-02-spec-pr-delivery-and-retro.md) still owns pull-request cardinality, the merger subagent, scratch notes, and the retro gate. [Runtime-specific executors](2026-08-27-runtime-specific-delivery-executors.md) still choose Codex versus DSH workers; sequential dispatch is sequential isolated writers, not the root writing.
