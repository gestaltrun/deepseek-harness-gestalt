...
Each documentation build owns its complete resolved `outDir` as disposable output. `buildDocumentationSite` fixes the build root to `website/`; the removal function and VitePress lifecycle callback remain private to `website/build.ts`, so no caller can ask this capability to remove an arbitrary path. The callback removes the resolved output after VitePress loads the fixed site's configuration and before the current bundle or `public/` files are written; a link-shaped output path is unlinked without traversing its target. Raw projection does not remove files at `buildEnd`, so its collision check sees every file produced by the current build and still rejects a public file, page, or image at the same path.
