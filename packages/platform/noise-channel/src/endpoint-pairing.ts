...
import { encodeRelayAuthorityEnvelope } from './relay-authority-envelope.ts'
...
export class SnowDesktopEndpointPairingOwner {
   ...
   * @param attachmentKey - Desktop-generated application secret delivered only inside Snow transport.
   * @returns opaque transport ciphertext for the mailbox.
   */
  async sealMobileRelayAuthority(grant: RelayCredentialGrant, attachmentKey: Uint8Array): Promise<Uint8Array> {
    ...
    const sealed = await sealPairingTransport({
      ...
      plaintext: encodeRelayAuthorityEnvelope(grant, attachmentKey),
    })
