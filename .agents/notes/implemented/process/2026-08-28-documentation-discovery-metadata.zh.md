...
产品站 `www.gestaltrun.com` 由当前仓库的 `apps/platform/public` 提供，并随 Platform 容器部署。文档构建与产品站根域拥有不同路由，因此各自必须为自己实际提供的 URL 发布元数据。
...
[Platform 官网发现元数据决策](2026-08-29-platform-homepage-discovery-metadata.zh.md)持有 `www.gestaltrun.com` 的元数据与站点根路径发现文件。
...
部署必须传入目标站点的完整基础 URL。Google 只在 domain 或 subdomain 级别分配 site name，项目 Pages 子路径下的 `robots.txt` 也无法控制 host root。Platform 官网持有站点根路径的产品信号；Pages 构建持有文档路由及其语言配对。
