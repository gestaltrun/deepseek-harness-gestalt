...
持久附件服务边界。`ctx.attachments` 校验并持久提交提供方无关的规范化图片或不可变通用文件字节，随后返回可序列化的内容寻址引用；消费方绝不会在会话事件中持久保存浏览器路径、对象 URL、提供方 URL 或 base64。`saveFile` 与 `readFile` 保留确切字节、有界 media type 与展示名称、字节数和 SHA-256。不支持通用文件的提供方会明确拒绝该操作。
...
- 通用文件进入模型、音频、视频与持久的未发送草稿仍需要单独的提供方契约；Session 范围的 Companion 文件准入只写入日志。
