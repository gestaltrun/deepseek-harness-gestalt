...
[CI](../../../../.github/workflows/ci.yml) 与 [CI master](../../../../.github/workflows/ci-master.yml) 为候选树、master push、按日和手动事件赋予互补的职责。候选证据在标准 hosted 容量上运行完整平台清单。每次 master push 会在公司自有 `vm-backup` 与 `dsh-win-ci` 池上运行有界的 Linux 与 Windows standby smoke；每日或 `standby-exhaustive` dispatch 会在这些池上运行完整的非分片参考流程。标准 hosted `serial / macos` 仍处于禁用状态，并由 `TODO(hosted-serial-ci)` 标记，直到其可移植容量恢复。各自独立的 job 定义会显式保留 checkout、运行时设置、干净安装和平台归属，而不是用矩阵或可复用 workflow 隐藏操作系统差异。
