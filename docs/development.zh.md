...
穷尽 plan 保留 Wine 作为必需 Windows 信号，并把原生 Windows 构建与 runtime、覆盖率和静态可移植性作为并行观察性证据运行。覆盖率使用两个生成 blob 的 worker，再由一个 job 合并并应用阈值；构建/runtime 与静态可移植性各自保留独立 worker。任一证据 owner 失败、取消或跳过时，单一原生 verdict 都会失败，但不会延迟 `all checks passed`。每个 owner 都会发布可检查证据；完整本地清单仍使用 `pnpm run check:ci:windows-complete`。
