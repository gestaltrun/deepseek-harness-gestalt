...
The GUI and Agent coordinate vocabulary stays in capture pixels on both platforms. Android forwards those pixels unchanged. XCTest consumes logical points, so the first iOS tap or gesture reads the official `device.info.screenSize`, validates its positive finite width, height, and scale, caches that size for the mobilecli generation, and converts every tap or gesture `x`/`y` coordinate before forwarding. Landscape versus portrait WDA bounds follow the live capture surface ([orientation](../bug-fix/2026-09-04-ios-landscape-tap-orientation.md)). Generation replacement clears the cache. A missing or malformed iOS screen size fails as `PHONE_PROTOCOL` instead of returning a successful no-op.
