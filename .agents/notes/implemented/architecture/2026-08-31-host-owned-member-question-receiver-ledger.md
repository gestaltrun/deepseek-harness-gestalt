...
The environment ledger is the authority for pending and terminal projections. It stores the bounded Decision Brief fields admitted by the Companion codec, reference path/reason metadata, routing identities, terminal metadata, and each reserved human action as text plus durable attachment references under its request digest. Referenced document bodies and raw browser image bytes are absent. Startup validates the complete document through the current Companion codec and fails on a foreign format, malformed record, dangling reference, inconsistent terminal, or mismatched admission digest.
