...
主窗口只接受当前环回 Host 同源导航。包括 GitHub 授权在内的普通 HTTP 链接交给系统浏览器；其他来源和 scheme 不能替换 Session Surface，也不能创建另一个 Electron 窗口。Platform 账号签名密钥和令牌保存在 Electron userData 下的 `safeStorage` 加密文件中；preload 只暴露当前状态与生命周期动词。Account 的 `beginLogin` 立即返回当前快照，因此 Settings 不会在 Host Account HTTP 期间被堵住。Account、Personal Pairing 与 updater 快照会同时投影到主 Session Surface 和当前 Settings overlay；已销毁或已替换的 overlay 不再接收后续事件。
