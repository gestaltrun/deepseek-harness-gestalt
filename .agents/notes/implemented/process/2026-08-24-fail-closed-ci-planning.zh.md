...
pull-request workflow 通过单一 `preflight` job 准入所有证据 job。昂贵 job 启动前，它会校验 workflow 语义、计算 plan、要求 PR 元数据包含全部受影响 area，并检查 immutable install、生成目录、翻译配对、模块图与 workspace constraints。
