...
Desktop replacement keeps the opened Mobile conversation and cached rows visible, disables mutation controls while offline, and restores the same encrypted channel only after Host authority is ready. Desktop window loading is independent of Relay readiness, while shutdown still drains the controller-owned background start. A foreground disconnect cannot throw `Companion history requires foreground synchronization` through React. Host failures after an established Desktop remains online are still application data rather than Relay disconnects.
