...
describe('bundled Mobile product entry', () => {
  it.each([
    ...
    expect(await page.getByRole('heading', { name: 'Paired Desktops' }).count()).toBe(1)
    expect(await page.getByRole('button', { name: /Authenticated Shared Desktop/ }).getAttribute('aria-pressed')).toBe('true')
    expect(await page.getByRole('button', { name: /Secondary Desktop/ }).getAttribute('aria-pressed')).toBe('false')
