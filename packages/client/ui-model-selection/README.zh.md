...
模型选择插件（浏览器侧）：**两个入口共用一份会话级目录**，由 `ModelDirectoryResolver`（`ctx.modelDirectories`）持有。`/model` popupSelect 贡献项（经 `ctx.commandUi` 注册）与 composer 的具名 `conversation.input.model` slot 都通过 `ISessions.modelRoute()` 解析 Session 的模型路由，并共用一个 `ModelDirectory` 实例。普通 Session 使用 `session.models` 与 `session.selectModel`；功能准入适配器可以持有其 Agent 的路由。紧凑型 composer 触发器会打开两级 Model/Effort 菜单：模型仍按提供方分组，所选具体模型则提供由其适配器持有的推理强度名称、说明和默认值。`/model` 应用所选模型的默认推理强度，composer 随后可以选择任一已公布的推理强度。
...
目录按会话惰性解析（`ctx.modelDirectories.directoryFor(sessionId)`），随会话作用域一并 dispose（资源释放）。功能路由可以为自身 Agent 生命周期公开模型选择，包括为首次准入保留已经验证的临时选择。没有此类路由的 catalog 已寻址 subagent Session 不公开任一入口，因为绑定到 agent（智能体）的普通模型 RPC 会在直接 parent 继续执行路径之外激活持久化 child 历史。
...
间接影响：两个入口都通过已解析的 Session 模型路由提交完整的 `ModelSelection`；普通 Session 会在下一次提示词组装边界对其进行快照，Side Chat 会保留临时选择并在首次提交创建子 Agent 时安装它，后续请求采用所选提供方、模型与推理强度而运行中的步骤保留已组装选择，只有当请求头记录一次实际采用该选择的请求后选择才会持久化，菜单交互不会添加提示词内容。
...
- **无通用已寻址 subagent 选择**——没有功能自有模型路由的 catalog 已寻址 subagent 不公开独立的模型选择约定。
