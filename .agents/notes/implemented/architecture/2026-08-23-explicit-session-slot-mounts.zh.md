...
Slot 注册表以 `renderSessionSlot()`、ui-renderer 以 `mountSession()` 提供框架级入口，用于挂载一个已声明的非 root Session slot。挂载会通过既有 `UiSession` adapter 解析指定会话的标配 props，并在独立 React 根中渲染，同时不改变 `sessions.list.current`。既有声明账本、entry 边界、store、inject 接口与标准钩子绑定仍是该树内的权威。Session 创建与历史打开仍由 Controller 持有。
