...
Mobile 与 Desktop 产品入口使用的长期 Account、配对、附件与 Relay 服务，是已运营的 [`apps/platform`](../../../../apps/platform/README.zh.md) 监听进程。该进程会在监听前校验唯一生产身份与 TLS/WSS route，迁移 PostgreSQL 中的 Account、配对、route 与加密附件状态，并仅将 Redis 用于会过期的 attachment directory 和在线密文 coordination。产品 endpoint 通过经评审的 Snow XKpsk3 配对握手与全新的 IK 连接握手派生配对私有密钥，在确认前封装独立的 Mobile Relay authority，把每次重连绑定到当前 attachment generation，并在应用工作前发送带版本的前台同步 projection。assembled 测试会让 Desktop 与 Mobile 落到不同 Platform instance，覆盖 Relay 替换、无密文保留的精确离线失败、持久 authority 恢复，以及已交付 Desktop Host 到 Mobile Companion 的操作。临时 PostgreSQL 与 Redis 测试覆盖跨实例持久性、Lua TTL 与替换安全，以及不存在 Stream 或 List queue。物理 WKWebView 与 Android WebView 执行、云证书和 #43 已运营部署仍属于外部发布证据。
