...
Windows vitest coverage was not part of the original change: the repository's Windows CI lane owned build/static gates, and unit coverage ran on Linux. Coverage lanes now declare that their runner must provide `pwsh`, so configuration and test workers share that authoritative requirement instead of independently probing under load; a missing CI capability fails rather than silently skipping the suites. Development hosts without `pwsh` still self-skip the real-process suites and exclude their source from local coverage. The vitest `windowsUnsupportedPackages` exclusion narrows from `packages/shell/*` to the bash-requiring packages so the pwsh suites can also run natively on Windows development machines.
