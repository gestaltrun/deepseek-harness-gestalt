...
`DSH_HOME` 按共享路径策略解析：显式配置、`$DSH_HOME`，最后是 `~/.dsh`。会话日志只包含引用和经过校验的元数据，绝不包含这个宿主路径。`saveFile` 对不超过 `maxFileBytes` 的通用文件确切字节复用相同的原子内容寻址发布；`readFile` 校验 id、SHA-256、字节数、media type 与展示名称。`readImage` 与 `readFile` 都保留调用方取消语义。
