...
`DeepSeekHarness` 会保留其按需启动的运行时子进程，以便在多次调用之间复用。请像上例一样将其用作上下文管理器，或在使用完毕后显式调用 `close()`。运行时确认 `shutdown` 后，`close()` 最多等待 `shutdown_timeout_seconds`，让服务器拥有的 teardown（资源清理）、持久化和正常进程退出完成。未收到确认或等待退出超时时，SDK 会转入有界的 terminate／kill 清理，因此关闭操作不会无限期遗留子进程。
