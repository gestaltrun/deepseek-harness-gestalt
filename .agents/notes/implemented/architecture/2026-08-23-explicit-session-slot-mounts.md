...
The Slot registry exposes `renderSessionSlot()` and ui-renderer exposes `mountSession()` as framework entries for a declared non-root Session slot. The mount resolves one explicit Session standard-props bundle through `UiSession.adapter.resolve(sessionId)`, and the Session Controller's `openForRender()` opens that identity's history when it is published, without changing `sessions.list.current`. The existing declaration ledger, entry boundaries, stores, inject faces, and standard hook binding remain authoritative inside that tree.
