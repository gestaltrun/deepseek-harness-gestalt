...
每个拉取请求还会启动一个名称为 `windows node 24 / native complete` 的常规独立 `windows-native` 作业，默认运行在 `windows-latest` 上；`DSH_CI_FAILOVER_WINDOWS=selfhosted` 会把可信拉取请求重定向到 `dsh-win-ci`。[标准托管主路径决策](2026-08-18-standard-hosted-primary-ci.md)负责该选择器。[串行门禁决策](2026-08-23-hosted-windows-serial-gates.md)负责顶层并发。该作业为工作区符号链接启用开发人员模式，通过 `pnpm/action-setup` 提供仓库固定版本的 pnpm，在不传输 store 归档的情况下执行不可变安装，并在原生 PowerShell 下运行 `pnpm run check:ci:windows-complete`。门禁卡住时，120 分钟超时会为其设定上限。
