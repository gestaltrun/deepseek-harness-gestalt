...
- `sessionFor(id, agentManaged, preferredFormat)` — IO upgrade path, the device-class first encoding, and signed `mjpeg` and `h264` URLs whose query token expires after `tokenTtlMs`; `agentManaged` marks an iOS real-device session.
- `POST /phone/session` — mints those URLs for a device present in the latest listing; the `/api` trust fence runs first. Android devices and iOS real devices prefer H264. iOS simulators prefer MJPEG because mobilecli rejects their AVC capture requests. An iOS real device first runs `agentStatus`: missing installation answers `PHONE_AGENT_MISSING`, while a successful session carries `agentManaged: true` so a later picture failure can re-check the agent.
- `POST /phone/agent/status` and `POST /phone/agent/install` — detect, install, or force-reinstall the device-control agent for a listed iOS real device. Other device classes are rejected. Signing identity, provisioning-profile selection, Developer Mode, device unlock, and trust remain user-owned prerequisites. A missing `provisioningProfilePath` answers `PHONE_AGENT_PROFILE_REQUIRED` with a configuration action instead of a generic upstream failure.
