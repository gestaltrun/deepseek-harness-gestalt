...
Windows vitest 覆盖率不属于最初改动：仓库的 Windows CI 通道当时负责构建/静态门禁，单元覆盖在 Linux 上运行。现在覆盖率通道显式声明 runner 必须提供 `pwsh`，配置与测试 worker 因此共享这一权威要求，不再在负载下各自探测；CI 缺少该能力时直接失败，不会静默跳过套件。没有 `pwsh` 的开发主机仍自行跳过真实进程套件，并在本地覆盖率中排除其源码。vitest 的 `windowsUnsupportedPackages` 排除从 `packages/shell/*` 收窄为真正需要 bash 的包，使 pwsh 套件也能在 Windows 开发机上原生运行。
