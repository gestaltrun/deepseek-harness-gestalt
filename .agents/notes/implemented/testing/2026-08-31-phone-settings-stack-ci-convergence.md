...
Release-family fixtures create both the dynamic client bundle and a minimal valid source map before recording the artifact digest. Desktop overlay composed-boot coverage has a dedicated exclusive suite list, so the coordinator excludes it from concurrent partitions and the shared process-bound batch, then runs it once in a fresh instrumented process.
...
The repaired gates describe the shipped settings composition, validate a complete client artifact fixture, schedule real Host subprocess tests according to their resource ownership, and keep binary-discovery assertions independent from unrelated runner environment state. These changes affect test evidence only and do not authorize a release or merge to `master`.
