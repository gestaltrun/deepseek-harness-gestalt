...
成员提问接收 Session 起初只是用于渲染的 identity，其 record 始终归 Host 所有。`ReceivingQuestionBook` 只应用 revision 更高的完整 `host/member-question-snapshot` 帧，使用每条持久化 `ReceivingSessionId`，并通过一个 identity-stable 的对外 `SessionFace` 发布 pending wait 与 terminal record band。断连会保留最近的 Host 状态；重开的 stream 会重新建立 pending 与 terminal 行的基线，且不改变其 id。Client 绝不裁决 expiry、supersession、withdrawal 或 settlement，回答与拒绝动作会调用 `memberQuestion.settle`。在显式提交 human message 之前，该 face 没有 Chat、队列、历史、模型、命令或 skill route；其 prompt 只以本次 action 所有的 envelope `rpcId` 调用 `memberQuestion.admitHumanTurn`。失败会保留该 id，Client 重启后则从 `reservedAdmission` snapshot 恢复。成功后的 snapshot 携带 `hostSessionId`，把同一个 face 绑定到普通 Host Session；随后历史与模型所有的 route 可用，sidebar row 不会被替换，而后续 pending brief 仍会让下一次 human submit 经过 admission。
