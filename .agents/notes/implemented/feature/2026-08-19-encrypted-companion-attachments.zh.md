...
`@deepseek-ai/dsh-remote-attachments` 拥有 Platform 侧。进程内 provider 只保留为包测试 fixture；实际运行的应用挂载 PostgreSQL store，其事务化 digest、密文、pairing、过期、容量、consume 与 revoke 状态由多个 Platform 实例共享。HTTP 插件通过当前 Mobile Installation 证明与确切的已确认 pairing selector 鉴别每个请求；selector 本身不授予 authority。Platform 不会收到 endpoint key 或明文。consume 只在 HTTP 响应完成后删除 blob；中途写入失败会保留它以便重试。
