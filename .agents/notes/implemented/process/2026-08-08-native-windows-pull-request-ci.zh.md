...
Wine 保留必需 aggregate 现有的关键路径和 job 身份。`all checks passed` 变绿时，原生 Windows 分区或其 verdict 仍可能处于待处理或红灯状态，因此 branch protection 采用 Wine 结果，而评审者和后续自动化采用独立的原生 verdict。
