...
**ResizeObserver on the image instead of polling.** Rejected: ResizeObserver reports the layout box, which CSS drives; it never observes `naturalWidth`/`naturalHeight`, so a natural-size flip produces no signal.
...
`phone-connected-view.client.spec.tsx` requires the box ratio to follow the decoded H264 surface and flip live when the fake decoder emits a rotated frame, to hold the placeholder until the first MJPEG measurement, and to re-measure a rotated MJPEG stream on the poll cadence — each arm asserting the tap maps to coordinates in the current orientation. `phone-connection.client.spec.tsx` pins `surfaceSize()` publication, notify-only-on-change, and landscape tap mapping. `connected-view-styles.client.spec.ts` pins the ratio-var sizing with the 0.5 fallback and `object-fit: contain`.
