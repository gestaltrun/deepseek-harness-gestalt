...
`POST /phone/session` sends `{ deviceId, format: 'avc' }`. The live phase loads the signed `h264` URL through [the WebCodecs canvas playback module](../bug-fix/2026-08-30-ui-phone-h264-webcodecs-playback.md), then switches to the same session's signed MJPEG URL if H264 cannot paint a frame. The devbar names connecting, waiting for a decoded first frame, the painted encoding, reconnecting, or error from `PhoneConnectionController` plus `surfaceSize`; it never invents a measured fps.
