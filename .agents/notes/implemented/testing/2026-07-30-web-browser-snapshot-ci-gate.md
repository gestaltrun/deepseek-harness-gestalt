...
The consumer job owns the [single Linux build](../process/2026-07-30-independent-ci-consumer-build.md), so `apps/web/dist` and the package `lib/` directories remain in its workspace for the browser suite. On hosted runners, CI installs Chromium and its system dependencies at the Playwright version in the lockfile. On the persistent failover VM, the image owns the Linux system packages and CI installs only Chromium, avoiding per-run `apt` mutation. The default-branch cache producer saves the content-addressed pnpm store and Playwright directory under keys containing operating system, architecture, Node, pnpm, and lockfile identity. Pull requests restore those namespaces without upload on the required path and retain unconditional provisioning for a cold or stale cache. The daily or manually dispatched exhaustive Linux standby runs the same comparison without hosted cache actions; the bounded standby smoke on each default-branch push proves the build and Browser Runtime paths without running the full browser comparison.
