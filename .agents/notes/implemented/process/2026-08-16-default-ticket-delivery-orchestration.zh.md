...
规格分支是集成点，也是进入 `master` 的唯一 pull request。Ticket 会保持开启直到该次合并。上游变化只通过 merger 拥有的同步点进入，清理则等待不存在唯一工作丢失风险的证据。自动合并仍会提高 ticket 范围、仓库检查和实时评审状态检查的准确性要求。用户的明确限制始终有效；无法提供隔离时，执行方式降级为顺序 Worker；发布工作始终在授权前暂停。
