...
The Desktop-only composition activates `session-query-sqlite` with `openAt: first-search` and a derived index at `DSH_HOME/session-search.sqlite`; browser `dsh web` retains the repository default `openAt: never`. Companion `search-sessions` operations call the Web Host `session.search` method. Mobile renders every correlated `session-search` Session id/snippet pair directly, including hits absent from the bounded Session baseline and Companion Cache, and never adds cached title, Workspace, summary, transcript, or substring matches. Selecting such a hit opens its id-backed detail immediately and requests history; Desktop resolves its running state from the exact row in the complete Host `session.list` response instead of the bounded baseline projection.

The Encrypted Companion Protocol owns `operation-failed` as the lossless Host failure result. Its closed categories are HTTP status, invalid wire response, typed business error, and timeout. HTTP failures retain the numeric status, including 400; business failures retain a bounded code and message; every failure carries the originating operation id. The Desktop loopback client settles a non-2xx response as soon as its headers arrive, before response-body accumulation and the absolute wall-clock deadline can replace the known status. It validates successful RPC envelopes and echoed ids. Ordinary unary calls use the configured accumulator capped by the 60 KiB Companion application-message ceiling. `session.history`, `session.list`, and `workspace.list` may instead consume a larger fixed projection-source bound derived from the transcript page byte and entry limits because Desktop narrows those complete Host values before Encrypted Companion encoding; attachments use their independently derived admission bound. Cumulative overflow at the applicable bound destroys the response and becomes an invalid-wire failure. The client returns values rather than throwing these expected failures. The Mobile surface accepts results only through a receiver bound to the decoder's physical connection generation. It correlates search results with the current search operation and owns exactly one unresolved attachment operation id returned by the mutation channel. A sending or uncertain attachment blocks another selection until confirmation, rejection, failure, or reconciled status releases the id; results for every other attachment id are inert. Attachment send completion can publish the uncertain state that retains the id for reconciliation. Search and attachment failures are visible alerts; disconnection, replacement, or backgrounding makes the old decoder receiver inert.
