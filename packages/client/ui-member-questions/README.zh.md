...
材料芯片是聚焦按钮：点击一枚芯片会通过可选的 `detailsFocus` 服务把该文档写入会话的详情面板，`conversation.details.document` 席位按扩展名分发——markdown 正文走 MarkdownText，html 正文走沙箱化受限预览，其余一律作为纯文件标签。详情面板打开期间卡片折叠为窄条，观察方式与最小化开关相同：通过持久详情列的 `aria-expanded`。
...
- **到期倒计时在接收端仅为展示** —— 卡片渲染决策简报的 `expiresAt` 时刻，但接收侧没有计时器让已展示的卡片失效；到期在发送端与被路由的提问本身上强制执行。
