...
The workflow downloads one immutable acme.sh source archive and verifies its SHA-256 before execution. ACME account and domain state is archived and encrypted client-side with AES-256-CBC and PBKDF2 before storage at one exact object in the existing private deployment OSS bucket. The encryption key is a protected Environment secret. Temporary state is owner-only and deleted when the job exits.
...
**A dedicated KMS key and managed scheduler.** Rejected because it adds a paid cloud service and another execution surface when the protected GitHub Environment already provides OIDC, encrypted secret storage, scheduling, and failure reporting.
...
Renewal depends on GitHub Actions and one protected Environment encryption secret, but Alibaba Cloud authorization remains short-lived and federated. Loss of the encryption secret requires a controlled ACME account re-registration or restoration from the protected operator copy; it does not expose an Alibaba Cloud credential. The previous CAS certificate remains available for explicit rollback and lifecycle cleanup is a separate reviewed operation.

## Verification

Platform workflow tests pin OIDC permissions, immutable ACME source verification, due and validation modes, encrypted owner-only state, challenge cleanup, key/SAN/lifetime validation order, listener-only mutation, and old-certificate retention. The operated dry-run validates current TLS and cloud read paths without certificate issuance.
