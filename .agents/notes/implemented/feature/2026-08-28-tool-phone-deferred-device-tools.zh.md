...
移动设备 dock（#355）需要 `ctx.phoneDevices` 上的模型 Consumer。把二十多个平行的 `android_*`/`mobile_*` schema 放进初始请求会重演 Tandem 257 工具失败；任意 `adb shell` 路径则等于把真机 root shell 交给模型。有后果的点按、boot 与 shutdown 还需要默认向人询问，且不能把产品策略写进设备群 Service。
