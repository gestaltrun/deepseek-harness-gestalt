...
声明完成后，配置模型步骤会从同一个 Models 联接快照得出首次运行就绪状态。只要用户已经能触达**任何**一个提供方，它就直接完成而不渲染——已注册且其具名凭据引用已存储的路由（包括来自启动环境且只读的凭据），或 profile 根本不指名引用、因而走原生认证的路由。只有二者皆无的用户才会被带到「设置 → 模型」。「去配置」完成本轮协调并打开 Models 分节；「稍后配置」只完成这一轮。联接失败或设置只读时，该步骤不渲染并直接完成；Models 页仍是诊断界面。
...
- **凭据清理范围刻意保持狭窄**：删除一行时，仅当其引用与页面派生的 `<ROUTE>_API_KEY` 目标完全一致，或官方 DeepSeek 的 profile 使用 `DEEPSEEK_API_KEY` 时，才会清除已配置且可写的凭据。自定义引用、环境凭据和无法识别的目标会保留，因为该行无法证明自己拥有它们。官方 DeepSeek 删除后会离开行列表（残留的空用户层分节和该密钥被清掉）。它不会再出现在「添加提供方」里。目录 `deepseek` 仍在该列表中。已配置的目录 `deepseek` 会写入 `DEEPSEEK_API_KEY`，因此未占用的官方行不会跟在旁边。官方适配器仍保持挂载。
