...
- **Image blocks stay in-process** — only `spawn` and `fork` advertise the `images` capability. Out-of-process backends (`acp`, `codex`, `claude-code`, `dsh-sdk`) reject an image-bearing prompt until their wire provably preserves image blocks end to end.
