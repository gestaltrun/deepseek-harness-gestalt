...
Host cleanup relies on a container ownership label written during bootstrap activation; losing that label makes cleanup fail closed and leaves durable recovery state for an operator. The EIP pair is a narrow dispatch input rather than a global inventory and remains valid only for the reviewed bootstrap transaction. Infrastructure readiness remains external: outbound HTTPS must actually reach the registry and staged artifacts, PostgreSQL and Redis must remain reachable, and certificates plus EIP routing must be ready before dispatch. NAT, routes, allowlists, or successful TCP probes alone do not mark bootstrap ready, and this workflow does not provision or repair them.
