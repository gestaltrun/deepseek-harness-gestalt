...
- `ingest(envelope)` accepts a decoded `member-question` operation and any completely reassembled referenced documents only beside the receiver Account authority established by the authenticated endpoint. Document count, paths, and byte ceilings must align exactly with the operation references. Replay of the same question is idempotent; conflicting authority or content fails. The Host creates and persists an opaque `ReceivingSessionId` for `(originSessionId, receiver Account)` and never derives an id from `mq-recv` or trusts an addressee inside plaintext.
...
The Provider writes one owner-only JSON document at `<storagePath>/<environment>/member-question-receiver.json` through random-sibling atomic replacement. Pre-release format version `2` stores bounded origin, background, question/options, reference path/reason metadata, canonical base64url document bytes, route identity, terminal metadata, exact Account/Project Workspace bindings, and each reserved human action as text plus durable attachment references under a SHA-256 request digest. Markdown and HTML bytes project as inline UTF-8 content for the details panel; every other file type remains byte-exact without an inline text projection. Raw browser image bytes remain outside this ledger.
