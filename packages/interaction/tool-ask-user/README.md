...
- `to_project_member` — optional single addressee. Copy `project_members.displayName` from a row whose `self` is false (public GitHub login); `accountId` and the asking account (`self: true`) are ineligible and fail with `SELF_ADDRESSEE`. When present, the call is routed through `ctx.memberQuestionSender` and never reaches the local user-questions provider. Runtime eligibility hides this parameter from assembled prompts unless `boundProjectResolver` returns a cloud-project id; the static registry schema still retains it.
