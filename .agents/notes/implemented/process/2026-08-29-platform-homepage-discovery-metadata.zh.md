...
生产产品域名 `https://www.gestaltrun.com/` 从 Platform 容器提供 `apps/platform/public/index.html`。原始 HTML 只有标题，产品链接仍指向此前的个人仓库 owner。搜索引擎与 Agent 无法获得 canonical URL、description、社交预览、结构化产品身份、爬虫策略、sitemap 或简短的机器可读产品说明。
...
Platform 测试读取受版本控制的元数据，启动组装后的产品入口，并请求发现文件。Platform Dockerfile 继续复制完整 public 目录，因此测试与部署使用同一份文件来源。
