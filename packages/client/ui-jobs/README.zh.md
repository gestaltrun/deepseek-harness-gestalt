...
Escape 关闭列表并把焦点交还触发器，在其外部按下指针同理。列表通过 portal 挂到 viewport，右边缘与触发器对齐，并在 viewport 内向左钳制，因此不会被窄 Side Chat 裁切。最后一个任务消失时先关闭列表再卸载控件，焦点因此不会从一个被移除的节点上凭空消失。样式只用 token；文案走本包自己的 `job` locale 命名空间。行为由 [Web 后台任务展示 Agent Note](../../../.agents/notes/implemented/feature/2026-08-08-web-background-job-display.zh.md) 规定。
