...
`ci-master.yml` sets `PNPM_CONFIG_OPTIONAL=true` for every job. The complete standby aggregates execute product tests and static tools whose platform executables are optional dependencies, so runner-global pnpm configuration cannot disable their installation.
