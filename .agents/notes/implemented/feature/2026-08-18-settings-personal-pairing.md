...
The existing Desktop `手机配对` Settings section owns the Mobile Access toggle, QR/full-link challenge, authentication words, confirmation, rejection, and paired-device list. Before sign-in, the Platform Account reminder owns the section header. After sign-in, the authenticated GitHub avatar and name replace that reminder instead of appearing as a second identity, and the accompanying text describes sign-out and retained phone pairings without Installation terminology. Paired-device status uses separate text, surface, border, and indicator colors. Chinese presentation uses Chinese task language; product names such as Platform and GitHub remain unchanged. QR generation uses the maintained, zero-dependency `uqr` encoder. Mobile accepts the same complete link through paste or browser-camera QR scanning and waits for Desktop confirmation. No new Session header, sidebar, approval, composer, or offline presentation is registered.
