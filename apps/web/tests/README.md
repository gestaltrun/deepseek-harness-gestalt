...
`member-question-receiving.e2e.ts` uses a mock remote Agent only as the sender identity, then exercises the shipped Host receiver, API Proxy, WebSocket mux, Client Runtime, dynamic module table, receiving sidebar row, member-question composite card, shared question presentation, and Host settlement RPC. Arrival materializes one Host Session in the invitation-bound Workspace and injects the Decision Brief without starting a model turn; a local answer leaves no permanent answered strip. Reference chips open the receiver-owned hidden Workspace copy through Better Sidebar Files and leave a same-named Workspace file untouched.
