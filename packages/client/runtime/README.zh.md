...
成员提问接收 Session 在完整生命周期内都仅用于渲染。`ReceivingQuestionBook` 为每个路由键持有一个对外 `SessionFace`，并通过其快照发布一个真实的 pending question wait；wait 保留发起方协议 Session id，face 使用确定性 `mq-recv:*` renderer id。该 face 没有 Chat、队列、历史、模型、命令、skill 或变更路由，其唯一可注入 timer 指向最早的活跃过期时刻。断连会结算该代的自有 wait 并取消 timer；回放会为仍处于 pending 的请求新建载体。选中、重连与 renderer 打开都不会为该身份创建或刷新 Host Session。
